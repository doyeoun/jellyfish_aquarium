...
JELLY_NAMES = ['파란 해파리', '분홍 해파리', '개구리 모자 해파리',
               ...
               '선인장 해파리', '눈사람 해파리', '황금 해파리', '무지개 해파리', '파분 해파리', '푸딩 해파리', '소다맛 푸딩 해파리', '벚꽃 해파리', '쌍둥이 해파리', '아기 해파리', '악마 해파리', '혼돈 해파리', '새싹 해파리', '해파리인 척 해파리', '홀로그램 해파리', '슬픈 해파리', '똥 해파리', '희노애락 해파리']
...
CULT_DOC_DESCS = {
    ...
    4: '작고 따뜻한 기운이 담긴 배양서. 아기 해파리를 교배로 만들 수 있다.',
    5: '빛과 분노가 뒤엉킨 배양서. 악마 해파리를 교배로 만들 수 있다.',
    6: '빛과 어둠이 뒤엉켜 탄생한 배양서. 천사와 악마가 하나가 된 혼돈 해파리를 만들 수 있다.',
    7: '웃음과 눈물이 공존하는 배양서. 기쁨과 슬픔을 동시에 품은 희노애락 해파리를 만들 수 있다.',
}
CULT_DOC_RESULT = {
    1: 22,    # → 무지개 해파리
    2: 23,    # → 파분 해파리
...
CULT_DOC_RECIPE = {
    ...
    4: (2,  5),   # 개구리 모자 해파리 + 고양이 해파리 → 아기 해파리
    5: (8, 15),   # 천사 해파리 + 화난 해파리 → 악마 해파리
    6: (8, 29),   # 천사 해파리 + 악마 해파리 → 혼돈 해파리
    7: (15, 34),  # 화난 해파리 + 슬픈 해파리 → 희노애락 해파리
}
...
JELLY_INFO = {
    ...
    29: {'habitat': '지옥의 심연, 뜨거운 용암 바다',
         'personality': '천사와 분노가 합쳐져 탄생했다. 겉은 차갑지만 속은 이글이글하다.',
         'quote': '"천사였던 게 수치스럽다."'},
    30: {'habitat': '빛과 어둠이 교차하는 경계의 바다',
         'personality': '선도 악도 아니다. 두 본성이 끝없이 싸우고 있지만 겉으로는 그저 조용히 떠다닌다.',
         'quote': '"나는 천사인가, 악마인가. 아마 둘 다이겠지."'},
    28: {'habitat': '따뜻하고 잔잔한 얕은 바다',
         'personality': '아직 세상이 낯설다. 모든 것이 신기하고 무섭지만 그래도 헤엄쳐 본다.',
         'quote': '"...응? ...응!"'},
    ...
    36: {'habitat': '기쁨과 슬픔이 공존하는 바다',
         'personality': '한쪽은 웃고 한쪽은 울고 있다. 어느 쪽이 진심인지는 본인도 모른다.',
         'quote': '"웃으면서 우는 게 제일 슬픈 거야."'},
    ...
    33: {'habitat': '빛이 굴절되는 깊은 바다 수면 아래',
         'personality': '온몸이 홀로그램처럼 일렁인다. 어떤 각도에서 봐도 다른 색이 보여서 포착이 어렵다.',
         'quote': '"내 진짜 색이 뭔지 나도 모르겠어."'},
    32: {'habitat': '강과 바다 사이 어딘가',
         'personality': '어릴 때부터 해파리들과 어울려 살았다. 다리를 달고 나니 이제 자기가 해파리인 줄 안다.',
         'quote': '"나 해파리 맞는데? 다리도 있잖아."'},
    31: {'habitat': '햇살 가득한 얕은 봄 바다',
         'personality': '막 싹을 틔운 것처럼 모든 것이 새롭고 신선하다. 조심스럽지만 자라고 싶은 마음이 가득하다.',
         'quote': '"이제 막 피어나는 중이야. 조금만 기다려줘."'},
}
...
def make_baby_bell_sprite():
    """아기 해파리: 작고 동글동글한 연핑크 외형."""
    art = [
        "......XXXX......",
        "....XXDDDDXX....",
        "...XDDMMMMDDX...",
    ...
    cmap = {'.':None,'X':(195,112,138),'D':(238,168,185),'M':(255,210,218),'H':(255,238,242)}
    ps = 4; W = 16; H = len(art)
    ...
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W]):
            ...
            if c: raw.set_at((col, row), (*c, 255))
    ec = (80, 30, 45, 255)
    raw.set_at((6, H-2), ec); raw.set_at((9, H-2), ec)
    return pygame.transform.scale(raw, (W*ps, H*ps))

BABY_BELL_SPRITE = make_baby_bell_sprite()

def make_demon_bell_sprite():
    """악마 해파리: #3d0000 기반 극도로 어두운 진홍색."""
    art = JELLY_DEFS[0]['art']
    cmap = {'.':None,'X':(18,0,42),'D':(42,0,91),'M':(62,5,118),'H':(85,15,145)}
    ps = 4; W = W_PIX; H = len(art)
    raw = pygame.Surface((W, H), pygame.SRCALPHA)
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W]):
            c = cmap.get(ch)
...
def make_chaos_bell_sprite():
    """혼돈 해파리: 왼쪽 흰색 계열, 오른쪽 어두운 회색 계열 — 픽셀별 반반."""
    art = JELLY_DEFS[0]['art']
    W = W_PIX; H = len(art); ps = 4
    # 왼쪽 흰색 음영 (H→X: 밝음→어둠)
    cmap_L = {'.':None, 'H':(252,252,252), 'M':(230,230,230), 'D':(200,200,200), 'X':(165,165,165)}
    # 오른쪽 어두운 음영
    cmap_R = {'.':None, 'H':(90,90,90),   'M':(60,60,60),   'D':(38,38,38),   'X':(20,20,20)}
    raw = pygame.Surface((W, H), pygame.SRCALPHA)
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W]):
            cmap = cmap_L if col < W//2 else cmap_R
            c = cmap.get(ch)
            if c: raw.set_at((col, row), (*c, 255))
    return pygame.transform.scale(raw, (W*ps, H*ps))

CHAOS_BELL_SPRITE = make_chaos_bell_sprite()

def make_sprout_bell_sprite():
    """새싹 해파리: #c9e669 연두색."""
    cmap_s = {'.':None,'X':(140,175,55),'D':(170,200,80),'M':(201,230,105),'H':(225,245,140)}
    art = JELLY_DEFS[0]['art']
    BH = len(art); ps = 4
    raw = pygame.Surface((W_PIX, BH), pygame.SRCALPHA)
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W_PIX]):
            ...
            if c: raw.set_at((col, row), (*c, 255))
    ey = BH - 3
    raw.set_at((W_PIX//2-2, ey), (60, 80, 20, 255))
    raw.set_at((W_PIX//2+1, ey), (60, 80, 20, 255))
    return pygame.transform.scale(raw, (W_PIX*ps, BH*ps))

SPROUT_BELL_SPRITE = make_sprout_bell_sprite()

def make_sad_bell_sprite():
    """슬픈 해파리: #3886f4 파란색, 초롱초롱한 눈."""
    cmap = {'.':None,'X':(20,70,180),'D':(40,105,220),'M':(56,134,244),'H':(120,185,255)}
    art = JELLY_DEFS[0]['art']
    BH = len(art); ps = 4
    raw = pygame.Surface((W_PIX, BH), pygame.SRCALPHA)
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W_PIX]):
            c = cmap.get(ch)
            if c: raw.set_at((col, row), (*c, 255))
    # 눈은 Jellyfish.draw에서 별도로 초롱초롱하게 그림 (스프라이트엔 없음)
    return pygame.transform.scale(raw, (W_PIX*ps, BH*ps))
...
def make_hna_bell_sprite():
    """희노애락 해파리: 왼쪽=분홍(기쁨), 오른쪽=파랑(슬픔) 반반."""
    art = JELLY_DEFS[0]['art']
    W = W_PIX; H = len(art); ps = 4
    cmap_L = {'.':None,'X':(180,155,0),'D':(220,190,20),'M':(255,225,50),'H':(255,245,140)}   # 노랑
    cmap_R = {'.':None,'X':(40,110,85),'D':(65,155,118),'M':(105,205,162),'H':(165,235,205)}  # #69cda2 청록
    raw = pygame.Surface((W, H), pygame.SRCALPHA)
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W]):
            cmap = cmap_L if col < W//2 else cmap_R
...
HNA_BELL_SPRITE = make_hna_bell_sprite()
...
def make_fake_jelly_sprite():
    ...
    W = (230, 242, 250)  # 눈 흰자
    _ = None             # 투명
    # 20×11 픽셀 그리드 — 통통하고 귀여운 픽셀아트 물고기
    #  꼬리=왼쪽, 머리=오른쪽
    g = [
        [_,_,_,_,_,_,F,F,F,F,F,_,_,_,_,_,_,_,_,_],  # 0 등지느러미 끝
        [_,_,_,_,_,F,H,H,H,H,F,_,_,_,_,_,_,_,_,_],  # 1 등지느러미
        [_,_,_,_,O,H,H,H,H,H,H,H,H,O,_,_,_,_,_,_],  # 2 몸통 상단 시작
        [T,_,_,O,H,H,H,H,H,H,H,H,H,H,H,O,_,_,_,_],  # 3 몸통 넓어짐
        [T,T,O,H,H,H,H,H,H,H,H,H,H,H,H,H,O,_,_,_],  # 4 몸통 최대폭
        [T,T,O,M,M,M,M,M,M,M,M,M,W,W,M,M,M,O,_,_],  # 5 눈 흰자
        [T,T,O,M,M,M,M,M,M,M,M,M,W,E,M,M,M,O,_,_],  # 6 눈 검자위
        [T,T,O,M,M,M,M,M,M,M,M,D,D,D,D,D,O,_,_,_],  # 7 하단 그림자
        [T,_,_,O,H,H,D,D,D,D,D,O,_,_,_,_,_,_,_,_],  # 8 몸통 바닥
        [_,_,_,_,_,F,F,F,F,O,_,_,_,_,_,_,_,_,_,_],  # 9 배지느러미
        [_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_],  # 10
    ]
    W_grid = len(g[0]); H_grid = len(g)
    surf = pygame.Surface((W_grid*ps, H_grid*ps), pygame.SRCALPHA)
    for row, line in enumerate(g):
        for col, col_val in enumerate(line):
            if col_val is not None:
                pygame.draw.rect(surf, col_val, (col*ps, row*ps, ps, ps))
    return surf

FAKE_JELLY_SPRITE = make_fake_jelly_sprite()

def make_amethyst_bell_sprite():
    """홀로그램 해파리: 깊은 보라 기반 + 홀로그래픽 줄무늬."""
    cmap = {'.':None,'X':(35,5,80),'D':(85,15,160),'M':(140,30,210),'H':(215,100,255)}
    art = JELLY_DEFS[0]['art']
    BH = len(art); ps = 4
    raw = pygame.Surface((W_PIX, BH), pygame.SRCALPHA)
    for row, line in enumerate(art):
        for col, ch in enumerate(line[:W_PIX]):
            c = cmap.get(ch)
            if c: raw.set_at((col, row), (*c, 200))
    ey = BH - 3
    raw.set_at((W_PIX//2-2, ey), (240, 180, 255, 255))
    raw.set_at((W_PIX//2+1, ey), (240, 180, 255, 255))
    return pygame.transform.scale(raw, (W_PIX*ps, BH*ps))

AMETHYST_BELL_SPRITE = make_amethyst_bell_sprite()
...
def make_demon_wings_sprite():
    ...
    cmap = {'.': None, 'X': (50, 0, 100)}
    W = 11; H = 6; ps = 4
    raw = pygame.Surface((W, H), pygame.SRCALPHA)
    for row in range(H):
        for col in range(W):
            ch = art[row][col] if col < len(art[row]) else '.'
            c = cmap.get(ch)
            if c:
                raw.set_at((col, row), (*c, 255))
    return pygame.transform.scale(raw, (W*ps, H*ps))

DEMON_WINGS_SPRITE = make_demon_wings_sprite()
...
WARDROBE_DROP_MAP = {
    ...
    28: ['baby'],              # 아기 해파리
    29: ['demon_wings'],       # 악마 해파리
    30: ['half_mask'],         # 혼돈 해파리
    31: ['sprout_item'],       # 새싹 해파리
    33: ['hologram_glasses'],  # 홀로그램 해파리
}
...
class AquariumFish:
    ...
    def draw(self, surf):
        ...
        spr = (RAINBOW_BELL_SPRITE if self.design_idx==22
               ...
               else BABY_BELL_SPRITE  if self.design_idx==28
               else DEMON_BELL_SPRITE if self.design_idx==29
               else CHAOS_BELL_SPRITE if self.design_idx==30
               else SPROUT_BELL_SPRITE if self.design_idx==31
               ...
               else HNA_BELL_SPRITE if self.design_idx==36
        ...
        if self.design_idx == 29:
            _draw_demon_wings(surf, x, y, bw, bh)
        if self.design_idx == 30:
            _draw_demon_wings(surf, x, y, bw, bh, wing_color=(240,240,240))
        if self.design_idx == 33:
            _draw_hologram_effect(surf, x, y, bw, bh, spr)
        else:
            surf.blit(spr, (x-bw//2, y-bh//2))
        _draw_slot_overlay(surf, self.design_idx, x, y-bh//2+4, bw, bh, happy=self.happy_timer>0)
        # 희노애락 눈 (어항)
        if self.design_idx == 36:
            _draw_hna_eyes(surf, x, y, bw, bh)
        ...
        # 혼돈 해파리 추가 효과 (어항)
        if self.design_idx == 30:
            _ct = pygame.time.get_ticks() * 0.001
            _flick = 0.55 + abs(math.sin(_ct * 7.1)) * 0.45
            # 여우불 (흰색)
            _n = 5
            for _i in range(_n):
                _a2 = _ct * 0.9 + _i * (math.pi * 2 / _n)
                _rx = bw * (0.68 + math.sin(_ct * 0.35 + _i * 1.8) * 0.22)
                _ry = bh * (0.58 + math.cos(_ct * 0.42 + _i * 2.1) * 0.20)
                _ox = x + int(math.cos(_a2) * _rx)
                _oy = y + int(math.sin(_a2) * _ry)
                _orb = max(2, int((2 + abs(math.sin(_ct * 4.3 + _i)) * 3) * _flick))
                for _gr in range(_orb*4,0,-2):
                    _ga2 = int(18*_flick*_gr//(_orb*4))
                    _gs2 = pygame.Surface((_gr*2+2,_gr*2+2),pygame.SRCALPHA)
                    pygame.draw.circle(_gs2,(220,220,220,_ga2),(_gr+1,_gr+1),_gr)
                    surf.blit(_gs2,(_ox-_gr-1,_oy-_gr-1))
                pygame.draw.circle(surf,(220,220,220),(_ox,_oy),_orb)
                pygame.draw.circle(surf,(255,255,255),(_ox,_oy),max(1,_orb-1))
            # 링 + 발광
            _hlw = int(bw*0.80); _hlh = max(1,int(_hlw*3//11))
            _hls = pygame.transform.scale(HALO_BASE,(_hlw,_hlh))
            _halo_y = y-bh//2-_hlh-3+int(math.sin(self.pulse)*3)
            for _ring in range(5,0,-1):
                _grw2 = _hlw+_ring*4; _grh2 = _hlh+_ring*2
                _ga3 = int(min(255,22*_flick*_ring//5))
                _gs3 = pygame.Surface((_grw2+4,_grh2+4),pygame.SRCALPHA)
                pygame.draw.ellipse(_gs3,(220,220,220,_ga3),(2,2,_grw2,_grh2))
                surf.blit(_gs3,(x-_grw2//2-2,_halo_y-_grh2//4))
            _wh = pygame.Surface((_hlw,_hlh),pygame.SRCALPHA)
            _mk = pygame.mask.from_surface(_hls)
            _wh.blit(_mk.to_surface(setcolor=(255,255,255,255),unsetcolor=(0,0,0,0)),(0,0))
            surf.blit(_wh,(x-_hlw//2,_halo_y))
            # 픽셀 도트 눈
            _ey3 = y + bh//8
            _pw3 = max(3, bw//12); _ph3 = max(3, bh//7)
            pygame.draw.rect(surf, (20,80,255),  (x-bw//8,  _ey3, _pw3, _ph3))
        ...
        elif self.design_idx == 30:
            # 혼돈: 왼쪽 2개 흰색, 오른쪽 2개 어두운 회색
            _amp2 = 2.0
            _chaos_cols = [(230,230,230),(230,230,230),(40,40,40),(40,40,40)]
            for _i2, _dx in enumerate([-0.28,-0.10,0.10,0.28]):
                _bx3 = x + int(_dx*bw)
                _base2 = int(bw*0.20)
                _wave2 = int(math.sin(self.tent_ph*1.8+_i2*1.3)*bw*0.06)
                _len2 = max(5,_base2+_wave2+int(_amp2*2))
                _sw2 = int(math.sin(self.tent_ph+_i2)*(1+_amp2*0.7))
                _s3 = pygame.Surface((14,_len2+10),pygame.SRCALPHA)
                _c2 = _chaos_cols[_i2]
                pygame.draw.line(_s3,(*_c2,195),(7,0),(7+_sw2,_len2),3)
                pygame.draw.circle(_s3,(*_c2,175),(7+_sw2,_len2),max(2,_len2//6))
                surf.blit(_s3,(_bx3-7,y+bh//2))
        else:
...
def draw_wardrobe_item_icon(surf, cx, cy, item_id, unlocked=True):
    ...
    if item_id == 'angel_halo':
        ...
        yc=c((255,210,30),(65,80,110)); yk=c((255,240,115),(75,90,120))
        ol=c((90,46,8),(45,55,85)); sc=c((38,78,16),(30,50,70))
        iy = cy + 1
        def _ip3(p0x,p0y,p1x,p1y,p2x,p2y,p3x,p3y,thick):
            pl=[]; pr=[]
            for i in range(8):
                t=i/7; mt=1-t
                qx=mt**3*p0x+3*mt**2*t*p1x+3*mt*t**2*p2x+t**3*p3x
                qy=mt**3*p0y+3*mt**2*t*p1y+3*mt*t**2*p2y+t**3*p3y
                dx=3*mt**2*(p1x-p0x)+6*mt*t*(p2x-p1x)+3*t**2*(p3x-p2x)
                dy=3*mt**2*(p1y-p0y)+6*mt*t*(p2y-p1y)+3*t**2*(p3y-p2y)
                mg=(dx**2+dy**2)**0.5 or 1; nx,ny=-dy/mg,dx/mg
                w=thick*(1-t*0.82)
                pl.append((qx+nx*w,qy+ny*w)); pr.append((qx-nx*w,qy-ny*w))
            pi=[(int(p[0]),int(p[1])) for p in pl+list(reversed(pr))]
            if len(pi)>=3:
                pygame.draw.polygon(surf,ol,[(p[0]+1,p[1]+1) for p in pi])
                pygame.draw.polygon(surf,yc,pi)
                hi=[(int(p[0]),int(p[1])) for p in pl[1:-1]]
                if len(hi)>=2: pygame.draw.lines(surf,yk,False,hi,1)
        sw=2.2
        # 왼쪽 1
        _ip3(cx-1,iy-1, cx-6,iy-1, cx-11,iy+2, cx-14,iy+0, sw)
        # 왼쪽 2
        _ip3(cx,  iy,   cx-5,iy+3, cx-9, iy+4, cx-11,iy+1, sw*0.78)
        # 오른쪽
        _ip3(cx+1,iy-1, cx+6,iy+2, cx+10,iy+3, cx+12,iy+0, sw*0.85)
        # 기저
        pygame.draw.circle(surf,ol,(cx,iy),3); pygame.draw.circle(surf,yc,(cx,iy),2)
        # 꼬다리
        ...
        _sf_icon(9, -7, 3)
        _sf_icon(0, 10, 3)
    elif item_id == 'demon_wings':
        wc_i = c((18,18,22),(35,40,60)); rc_i = c((200,30,30),(60,65,95))
        for sd_i in (-1,1):
            bx_i = cx + sd_i*4; w=14; h=8
            pts_i = [
                (bx_i,                   cy-int(h*0.45)),
                (bx_i+sd_i*int(w*0.38),  cy-h),
                (bx_i+sd_i*w,            cy-int(h*0.25)),
                (bx_i+sd_i*int(w*0.80),  cy+int(h*0.10)),
                (bx_i+sd_i*int(w*0.60),  cy+int(h*0.38)),
                (bx_i+sd_i*int(w*0.42),  cy+int(h*0.12)),
                (bx_i+sd_i*int(w*0.25),  cy+int(h*0.38)),
                (bx_i,                   cy+int(h*0.38)),
            ]
            pygame.draw.polygon(surf,wc_i,pts_i)
            pygame.draw.polygon(surf,rc_i,pts_i,1)
    elif item_id == 'half_mask':
        # 반원 마스크 아이콘
        mc = (220,220,220) if unlocked else (100,100,100)
        mc2 = (50,50,50) if unlocked else (40,40,40)
        # 왼쪽 흰색 반원
        pygame.draw.ellipse(surf, mc, (cx-12, cy-8, 12, 16))
        # 오른쪽 어두운 반원
        pygame.draw.ellipse(surf, mc2, (cx, cy-8, 12, 16))
        # 테두리
        pygame.draw.ellipse(surf, (150,150,150) if unlocked else (60,60,60), (cx-12, cy-8, 24, 16), 1)
        # 눈구멍 (왼쪽)
        pygame.draw.ellipse(surf, (30,30,40), (cx-9, cy-4, 5, 4))
    elif item_id == 'hologram_glasses':
        # 홀로그램 안경 아이콘 — 둥근 사각형 렌즈 + 홀로그래픽 줄무늬
        t_hg = pygame.time.get_ticks() * 0.001
        gy = cy + 2
        _ilw, _ilh, _irad = 11, 8, 3  # 렌즈 크기
        _holo_i = [(0,255,255),(50,255,160),(230,60,255),(255,60,200),(60,200,255)]
        _off_i = int((t_hg * 20) % _ilh)
        for _lxi in (cx - 13, cx + 3):
            _li_s = pygame.Surface((_ilw, _ilh), pygame.SRCALPHA)
            pygame.draw.rect(_li_s, (15, 8, 35, 120), (0, 0, _ilw, _ilh), border_radius=_irad)
            if unlocked:
                for _ci, _hc in enumerate(_holo_i):
                    _sy = (_ci * (_ilh // len(_holo_i)) - _off_i) % _ilh
                    for _dy in (_sy, _sy - _ilh):
                        if -2 < _dy < _ilh:
                            pygame.draw.rect(_li_s, (*_hc, 170), (0, int(_dy), _ilw, max(1, _ilh//5)))
            _clip_i = pygame.Surface((_ilw, _ilh), pygame.SRCALPHA)
            pygame.draw.rect(_clip_i, (255,255,255,255), (0, 0, _ilw, _ilh), border_radius=_irad)
            _li_s.blit(_clip_i, (0,0), special_flags=pygame.BLEND_RGBA_MULT)
            surf.blit(_li_s, (_lxi, gy - _ilh//2))
            pygame.draw.rect(surf, (220,220,255) if unlocked else (100,100,130),
                             (_lxi, gy-_ilh//2, _ilw, _ilh), 1, border_radius=_irad)
        # 브릿지 + 다리
        pygame.draw.line(surf, (200,200,255) if unlocked else (100,100,130), (cx-2, gy), (cx+3, gy), 1)
        pygame.draw.line(surf, (180,180,220) if unlocked else (80,80,100), (cx-13, gy), (cx-16, gy), 1)
        pygame.draw.line(surf, (180,180,220) if unlocked else (80,80,100), (cx+14, gy), (cx+17, gy), 1)
    elif item_id == 'sprout_item':
        # 새싹 아이콘 — _draw_sprout 스타일 (가로 타원 잎)
        _ic = (118, 202, 1) if unlocked else (80, 120, 40)
        _id = (72, 140, 0)  if unlocked else (50,  80, 20)
        _ih = (190, 235, 80) if unlocked else (120, 160, 60)
        # 줄기
        pygame.draw.rect(surf, _id, (cx-1, cy+4, 2, 8))
        # 잎: 가로가 긴 타원 두 개 (lw > lh)
        for _sx, _sy in ((-10, -1), (3, -1)):
            pygame.draw.ellipse(surf, _ic, (cx+_sx, cy+_sy, 10, 7))
            pygame.draw.ellipse(surf, _id, (cx+_sx, cy+_sy, 10, 7), 1)
            pygame.draw.ellipse(surf, _ih, (cx+_sx+2, cy+_sy+1, 5, 3))
    elif item_id == 'baby':
        # 큰 해파리(우) + 작은 해파리(좌) — PLAYER_BELL_SPRITE 축소
        main_w, main_h = 20, 15
        baby_w, baby_h = 12,  9
        # 메인 (우측)
        ms = pygame.transform.scale(PLAYER_BELL_SPRITE, (main_w, main_h))
        if not unlocked: ms.set_alpha(80)
        surf.blit(ms, (cx+1 - main_w//2, cy - main_h//2))
        # 아기 (좌측, 살짝 겹침)
        bs = pygame.transform.scale(PLAYER_BELL_SPRITE, (baby_w, baby_h))
        if not unlocked: bs.set_alpha(80)
        surf.blit(bs, (cx - main_w//2 - baby_w//2 + 4, cy - baby_h//2 - 3))
...
def draw_aquarium_add_screen(surf, inventory, scroll_y=0):
    ...
    for slot in range(len(JELLY_NAMES)):
        ...
        spr5 = (RAINBOW_BELL_SPRITE  if slot==22
                ...
                else DEMON_BELL_SPRITE  if slot==29
                else CHAOS_BELL_SPRITE  if slot==30
                else SPROUT_BELL_SPRITE if slot==31
                ...
                else HNA_BELL_SPRITE if slot==36
                else BELL_SPRITES[bi5])
        spr5 = pygame.transform.scale(spr5,(sw5,sh5))
        if slot==9:   spr5.set_alpha(72)
        elif slot==6: spr5.set_alpha(175)
        if slot==29: _draw_demon_wings(surf, cx5, cy5-CH//2+8+sh5//2, sw5, sh5)
        if slot==30: _draw_demon_wings(surf, cx5, cy5-CH//2+8+sh5//2, sw5, sh5, wing_color=(240,240,240))
        if slot==33:
            _draw_hologram_effect(surf, cx5, cy5-CH//2+8+sh5//2, sw5, sh5, spr5)
        else:
...
def draw_dev_spawn_screen(surf, dev_spawn_idx, scroll_y=0):
    ...
    bb = pygame.Surface((75,28),pygame.SRCALPHA); bb.fill((22,42,88,210))
    pygame.draw.rect(bb,(65,120,210),(0,0,75,28),1,border_radius=6); surf.blit(bb,(15,12))
    surf.blit(tb,(15+37-tb.get_width()//2, 12+14-tb.get_height()//2))
    ft = get_font(17, bold=True)
    nt = ft.render('[DEV] 스폰 필터 설정', True, (140, 180, 255))
    surf.blit(nt, (WIDTH//2-nt.get_width()//2, 14))
    # 전체 버튼
    all_sel = dev_spawn_idx is None
    ab = pygame.Surface((80,26),pygame.SRCALPHA)
    ab.fill((20,80,20,220) if all_sel else (12,30,12,180))
    pygame.draw.rect(ab,(60,200,80) if all_sel else (35,100,45),(0,0,80,26),1,border_radius=5)
    surf.blit(ab,(WIDTH//2-40,42))
    fa=get_font(12,bold=True); ta=fa.render('전체 (랜덤)',True,(200,255,200) if all_sel else (120,180,130))
...
def draw_online_world(surf, lx, ly, lnick, players, chat_msgs, chat_input, chat_active, chat_ime, move_phase=0.0, local_chat='', local_chat_t=0, interact_open=False, action=None, action_phase=0.0, selected=None, pushed=None, push_anim_t=0, push_dir=(1,0), equipped_item=None, self_pushed=None, status_msg='', show_status_input=False, status_input_text='', status_input_ime='', call_selected=None, chat_pinned=False, loc='main', cam_x=0, chat_scroll=0):
    ...
    for nick,data in players.items():
        ...
        _p_nick_extra={'hat':12,'deep_orb':16,'rabbit_ears':8,'frog_hat_item':8,'cherry_top':10,'angel_halo':10,'banana_peel':10,'apple':14,'cat_ears_item':7,'sprout_item':18,'crown':12}.get(data.get('equipped','') or '',0)
    ...
    if self_pushed:
        ...
        if equipped_item == 'demon_wings':
            _draw_demon_wings(surf, int(lx), int(ly), sp_sz, sp_h, bright=True)
    ...
    _nick_extra = {'hat':12,'deep_orb':16,'rabbit_ears':8,'frog_hat_item':8,'cherry_top':10,'angel_halo':10,'banana_peel':10,'apple':14,'cat_ears_item':7,'sprout_item':18,'crown':12}.get(equipped_item or '',0)
...
def draw_player_item(surf, cx, cy, bw, bh, item_id):
    ...
    if item_id == 'angel_halo':
        ...
        # 바나나 껍질: 꼬다리 위로, 조각들은 수평→처짐→끝 컬링
        hy  = cy - bh//2
        yc  = (255,210,30); yk = (255,242,120); ol = (90,46,8); sc = (38,78,16)
        L   = max(13, bw//2)
        sw  = max(2, bw//13)   # 더 얇게

        def _peel3(p0x,p0y, p1x,p1y, p2x,p2y, p3x,p3y, thick):
            pl=[]; pr=[]
            for i in range(11):
                t=i/10; mt=1-t
                qx=mt**3*p0x+3*mt**2*t*p1x+3*mt*t**2*p2x+t**3*p3x
                ...
                dy=3*mt**2*(p1y-p0y)+6*mt*t*(p2y-p1y)+3*t**2*(p3y-p2y)
                mg=(dx**2+dy**2)**0.5 or 1; nx,ny=-dy/mg,dx/mg
                w=thick*(1-t*0.82)   # 끝으로 갈수록 많이 얇아짐
                pl.append((qx+nx*w,qy+ny*w)); pr.append((qx-nx*w,qy-ny*w))
            pi=[(int(p[0]),int(p[1])) for p in pl+list(reversed(pr))]
            if len(pi)>=3:
                pygame.draw.polygon(surf,ol,[(p[0]+1,p[1]+1) for p in pi])
        ...
        # 왼쪽 조각 1 — 왼쪽 위 방향, 완만하게 처지다가 끝 컬링
        _peel3(cx-2,  hy-1,
               cx-int(L*0.40), hy-int(L*0.04),
               cx-int(L*0.78), hy+int(L*0.14),
               cx-int(L*1.00), hy+int(L*0.01),
               sw)
        # 왼쪽 조각 2 — 왼쪽 아래 방향으로 더 처짐, 길이 짧게 (명확히 분리)
        _peel3(cx-1,  hy+1,
               cx-int(L*0.35), hy+int(L*0.18),
               cx-int(L*0.60), hy+int(L*0.26),
               cx-int(L*0.75), hy+int(L*0.06),
               sw*0.78)
        # 오른쪽 조각 — 오른쪽으로 명확히 수평 뻗음, 얕게 처짐
        _peel3(cx+2,  hy-1,
               cx+int(L*0.42), hy+int(L*0.10),
               cx+int(L*0.68), hy+int(L*0.18),
               cx+int(L*0.82), hy+int(L*0.02),
               sw*0.85)

        # 중앙 기저
        br = max(4, bw//10)
        pygame.draw.circle(surf, ol, (cx,hy), br+1)
        pygame.draw.circle(surf, yc,  (cx,hy), br)
        pygame.draw.circle(surf, yk,  (cx-br//3, hy-br//3), max(1,br//2))
        ...
        # 온라인 플레이어 스프라이트를 마스크로 써서 왼쪽 절반만 흰색 오버레이
        _spr_m = pygame.transform.scale(PLAYER_BELL_SPRITE, (bw, bh))
        _ov_m = pygame.Surface((bw, bh), pygame.SRCALPHA)
        _ov_m.fill((235, 235, 235, 170), (0, 0, bw//2, bh))
        _mk_m = pygame.mask.from_surface(_spr_m)
        _ov_m.blit(_mk_m.to_surface(setcolor=(255,255,255,255), unsetcolor=(0,0,0,0)), (0,0), special_flags=pygame.BLEND_RGBA_MULT)
        surf.blit(_ov_m, (cx - bw//2, cy - bh//2))
        pygame.draw.line(surf, (200,200,200), (cx, cy-bh//2), (cx, cy+bh//2), 1)
        # 눈 색상: 원래 눈 지우고 새 색으로 교체
        px_w = max(3, bw//12); px_h = max(3, bh//7)
        ey_m = cy + bh//8
        # 왼쪽(흰 면): 흰색으로 기존 눈 지우고 파랑 도트
        pygame.draw.rect(surf, (235, 235, 235), (cx-bw//8-1, ey_m-1, px_w+2, px_h+2))
        pygame.draw.rect(surf, (20, 80, 255),   (cx-bw//8,   ey_m,   px_w,   px_h))
        # 오른쪽(살색 면): 살색으로 기존 눈 지우고 빨강 도트
        pygame.draw.rect(surf, (252, 210, 168), (cx+bw//16-1, ey_m-1, px_w+2, px_h+2))
        pygame.draw.rect(surf, (220, 20, 20),   (cx+bw//16,   ey_m,   px_w,   px_h))
    elif item_id == 'hologram_glasses':
        t_hg = pygame.time.get_ticks() * 0.001
        gy = cy + bh//8 + 3
        lr = int(bw * 0.19)  # 렌즈 반지름
        # 렌즈 위치 (간격 충분히)
        lx1, lx2 = cx - int(bw*0.28), cx + int(bw*0.28)
        # 홀로그래픽 렌즈 (둥근 사각형)
        _lw = lr*2 + lr//2  # 가로를 살짝 넓게
        _lh = lr*2
        _rad = lr//2        # 둥근 모서리 반지름
        for lx_c in (lx1, lx2):
            _lens = pygame.Surface((_lw, _lh), pygame.SRCALPHA)
            pygame.draw.rect(_lens, (15, 8, 35, 120), (0, 0, _lw, _lh), border_radius=_rad)
            _holo2 = [(0,255,255),(50,255,160),(230,60,255),(255,60,200),(60,200,255)]
            _offset2 = int((t_hg * 20) % _lh)
            _n_holo = len(_holo2)
            for _ci2, _hc3 in enumerate(_holo2):
                _sy2 = (_ci2 * (_lh//_n_holo) - _offset2) % _lh
                for _dy2 in (_sy2, _sy2 - _lh):
                    if -4 < _dy2 < _lh:
                        pygame.draw.rect(_lens, (*_hc3, 180), (0, int(_dy2), _lw, max(2,_lh//5)))
            # 둥근 사각형 마스크로 클리핑
            _clip = pygame.Surface((_lw, _lh), pygame.SRCALPHA)
            pygame.draw.rect(_clip, (255,255,255,255), (0, 0, _lw, _lh), border_radius=_rad)
            _lens.blit(_clip, (0,0), special_flags=pygame.BLEND_RGBA_MULT)
            surf.blit(_lens, (lx_c - _lw//2, gy - _lh//2))
            pygame.draw.rect(surf, (220, 220, 255), (lx_c-_lw//2, gy-_lh//2, _lw, _lh), max(1, bw//20), border_radius=_rad)
        # 브릿지
        pygame.draw.line(surf, (200, 200, 255), (lx1+_lw//2, gy), (lx2-_lw//2, gy), max(1, bw//20))
        # 다리 (양쪽)
        pygame.draw.line(surf, (180,180,220), (lx1-_lw//2, gy), (cx-bw//2, gy), max(1,bw//22))
        pygame.draw.line(surf, (180,180,220), (lx2+_lw//2, gy), (cx+bw//2, gy), max(1,bw//22))
        # 주변 별 파티클 2~3개 — 균등 분포
        _base_angles = [0.5, 2.2, 4.0]  # 약 120도 간격
        for _i, _ang in enumerate(_base_angles):
            _px = cx + int(math.cos(_ang) * bw * (0.65 + (_i % 2) * 0.15))
            _py = cy + int(math.sin(_ang) * bh * (0.55 + (_i % 3) * 0.12))
            _fl = 0.5 + 0.5 * math.sin(t_hg * (3.5 + _i * 0.9) + _i * 2.5)
            if _fl < 0.2: continue
            _sz = max(2, int(4 * _fl))
            _pr = int(150 + 105 * (0.5 + 0.5 * math.sin(t_hg * 0.8 + _i)))
            _pg = int(200 * (0.5 + 0.5 * math.sin(t_hg * 0.55 + _i * 1.3)))
            _pb = int(180 + 75 * (0.5 + 0.5 * math.sin(t_hg * 0.65 + _i * 0.9)))
            _al = int(220 * _fl)
            _ss = pygame.Surface((_sz*2+4, _sz*2+4), pygame.SRCALPHA)
            _sc2 = _sz + 2
            pygame.draw.line(_ss, (_pr,_pg,_pb,_al), (_sc2-_sz,_sc2), (_sc2+_sz,_sc2), max(2,_sz//2))
            pygame.draw.line(_ss, (_pr,_pg,_pb,_al), (_sc2,_sc2-_sz), (_sc2,_sc2+_sz), max(2,_sz//2))
            surf.blit(_ss, (_px-_sc2, _py-_sc2))
    elif item_id == 'sprout_item':
        # 유저 해파리 머리 위 새싹
        _bob_s = int(math.sin(pygame.time.get_ticks()*0.001*1.2)*2)
        _draw_sprout(surf, cx, cy - bh//2 + int(bh*0.08) + _bob_s, bw, bh)
    elif item_id == 'baby':
        # 유저 해파리를 그대로 복제해 좌측 상단에 붙임
        _t   = pygame.time.get_ticks() * 0.001
        _bob = int(math.sin(_t * 2.1) * 2)
        _bw  = int(bw * 0.55); _bh = int(bh * 0.55)
        _bx  = cx - bw//2 - _bw//2 + 6
        _by  = cy - bh//4 + _bob
        # 플레이어와 동일한 벨 스프라이트를 축소
        _spr = pygame.transform.scale(PLAYER_BELL_SPRITE, (_bw, _bh))
        surf.blit(_spr, (_bx - _bw//2, _by - _bh//2))
        # 촉수 (온라인과 동일한 slime drip 스타일)
        _tc  = (192, 128, 92)    # PLAYER_BELL_SPRITE 살색 촉수
        _ty  = _by + _bh//2
        for _i, _dx in enumerate([-0.28,-0.10,0.10,0.28]):
            _tx  = _bx + int(_dx*_bw)
            _tl  = max(4, int(_bh*0.28))
            _sw2 = int(math.sin(_t*1.8+_i*1.3)*1.5)
            _ts  = pygame.Surface((10, _tl+6), pygame.SRCALPHA)
...
def _spawn_bat(x, y):
    _bat_particles.append({
        ...
        'sz': random.uniform(4, 7),
    })

def _draw_update_bats(surf):
    global _bat_particles
    t = pygame.time.get_ticks() * 0.001
    next_bats = []
    for b in _bat_particles:
        b['life'] -= 0.025
        ...
        next_bats.append(b)
        # 그리기
        flap = abs(math.sin(t * 9 + b['flap']))
        sz = b['sz']; al = int(b['life'] * 210)
        x2, y2 = int(b['x']), int(b['y'])
        wh = max(1, int(sz * 0.7 * flap))
        bs = pygame.Surface((int(sz*2)+2, wh+2), pygame.SRCALPHA)
        bc = (18, 18, 22, al)
        # 왼 날개
        pygame.draw.polygon(bs, bc, [(int(sz),wh),(0,0),(int(sz*0.5),wh)])
        # 오른 날개
        pygame.draw.polygon(bs, bc, [(int(sz),wh),(int(sz*2),0),(int(sz*1.5),wh)])
        surf.blit(bs, (x2-int(sz), y2-wh))
    _bat_particles[:] = next_bats

_sparkle_particles = []

def _spawn_sparkle(x, y):
    _ang = random.uniform(0, math.pi * 2)
    _spd = random.uniform(0.5, 2.0)
    _sparkle_particles.append({
        'x': float(x) + random.uniform(-12, 12),
        'y': float(y) + random.uniform(-10, 10),
...
def _draw_update_sparkles(surf):
    global _sparkle_particles
    next_sp = []
    for s in _sparkle_particles:
        s['life'] -= 0.025
        if s['life'] <= 0: continue
        s['x'] += s['vx']; s['y'] += s['vy']
        next_sp.append(s)
        sz = max(1, int(s['sz'] * s['life']))
        al = int(s['life'] * 220)
        gs = pygame.Surface((sz*2+2, sz*2+2), pygame.SRCALPHA)
        pygame.draw.circle(gs, (255, 255, 255, al), (sz+1, sz+1), sz)
        surf.blit(gs, (int(s['x'])-sz-1, int(s['y'])-sz-1))
    _sparkle_particles[:] = next_sp


def _draw_hologram_effect(surf, cx, cy, bw, bh, spr):
    """홀로그램 해파리 효과: 후광 + 줄무늬 + 파티클을 spr에 적용 후 blit."""
    t = pygame.time.get_ticks() * 0.001
    # 1. 후광 (뒤 레이어)
    gr2 = int(60  + 195 * (0.5 + 0.5 * math.sin(t * 0.7 + 0.5)))
    gg2 = int(215 * (0.5 + 0.5 * math.sin(t * 0.5 + 2.5)))
    gb2 = int(80  + 175 * (0.5 + 0.5 * math.sin(t * 0.6 + 4.5)))
    for _gr in range(3, 0, -1):
        _ga = int(22 + 10 * abs(math.sin(t * 0.9 + _gr)))
        _gs = pygame.Surface((bw + _gr*6, bh + _gr*4), pygame.SRCALPHA)
        pygame.draw.ellipse(_gs, (gr2, gg2, gb2, _ga), (0, 0, bw+_gr*6, bh+_gr*4))
        surf.blit(_gs, (cx-bw//2-_gr*3, cy-bh//2-_gr*2))
    # 2. 홀로그래픽 줄무늬 적용
    _scaled = spr.copy()
    _sw2, _sh2 = _scaled.get_size()
    _stripe_surf = pygame.Surface((_sw2, _sh2), pygame.SRCALPHA)
    _holo = [(0,220,220),(80,255,190),(200,130,255),(255,130,210),(120,200,255)]
    _n = len(_holo)
    _sh_s = max(4, _sh2 // _n)
    _offset = int((t * 18) % _sh2)
    for _ci, _hc in enumerate(_holo):
        _sy = (_ci * _sh_s - _offset) % _sh2
        for _dy in (_sy, _sy - _sh2):
            if -_sh_s < _dy < _sh2:
                pygame.draw.rect(_stripe_surf, (*_hc, 75), (0, _dy, _sw2, _sh_s * 3 // 5))
    _scaled.blit(_stripe_surf, (0, 0), special_flags=pygame.BLEND_RGB_ADD)
    surf.blit(_scaled, (cx - bw//2, cy - bh//2))
    # 3. 반짝이 파티클
    for _i in range(6):
        _seed = _i * 137.5
        _px2 = cx + int(math.cos(_seed) * bw * (0.5 + (_i % 3) * 0.15))
        _py2 = cy + int(math.sin(_seed * 0.7) * bh * (0.4 + (_i % 4) * 0.12))
        _fl2 = 0.5 + 0.5 * math.sin(t * (3.0 + _i * 0.7) + _i * 2.3)
        if _fl2 < 0.15: continue
        _sz2 = max(2, int(4 * _fl2))
        _pr = int(150 + 105 * (0.5 + 0.5 * math.sin(t * 0.8 + _i)))
        _pg = int(200 * (0.5 + 0.5 * math.sin(t * 0.55 + _i * 1.3)))
        _pb = int(180 + 75 * (0.5 + 0.5 * math.sin(t * 0.65 + _i * 0.9)))
        _al2 = int(200 * _fl2)
        _sp_s = pygame.Surface((_sz2*2+4, _sz2*2+4), pygame.SRCALPHA)
        _c2 = _sz2 + 2
        pygame.draw.line(_sp_s, (_pr,_pg,_pb,_al2), (_c2-_sz2,_c2), (_c2+_sz2,_c2), max(2,_sz2//2))
        pygame.draw.line(_sp_s, (_pr,_pg,_pb,_al2), (_c2,_c2-_sz2), (_c2,_c2+_sz2), max(2,_sz2//2))
        surf.blit(_sp_s, (_px2-_c2, _py2-_c2))


def _draw_hna_eyes(surf, cx, cy, bw, bh):
    """희노애락 해파리: ^ ∨ 눈 + 오른눈 눈물 + 왼눈 꽃 파티클."""
    t = pygame.time.get_ticks() * 0.001
    ey = cy + bh//8
    ex_l = cx - int(bw*0.17)
    ex_r = cx + int(bw*0.15)
    er = max(3, bw//12)
    ew = max(3, er//2+1)
    hw = max(3, er); hh = max(2, er//2)
    # ^ 왼눈
    _lpts = [(ex_l-hw, ey+hh//2),(ex_l, ey-hh),(ex_l+hw, ey+hh//2)]
    pygame.draw.lines(surf, (30,15,5), False, _lpts, ew)
    # ∨ 오른눈
    _rpts = [(ex_r-hw, ey-hh//2),(ex_r, ey+hh),(ex_r+hw, ey-hh//2)]
    pygame.draw.lines(surf, (30,15,5), False, _rpts, ew)
    # 오른눈 눈물
    for _di in range(2):
        _td = (t*1.5+_di*0.8)%2.0
        if _td>1.2: continue
        _dy = ey+bh//6+int(_td*bh*0.55)
        _da = int(180*(1-_td/1.2)); _dr = max(2,bw//18)
        _ts = pygame.Surface((_dr*2+2,_dr*3+2),pygame.SRCALPHA)
        pygame.draw.ellipse(_ts,(185,213,252,_da),(1,_dr//2,_dr*2,_dr*2))
        pygame.draw.polygon(_ts,(185,213,252,_da),[(_dr+1,0),(_dr-1,_dr),(_dr+3,_dr)])
        surf.blit(_ts,(ex_r-_dr-1,_dy-_dr//2))
    # 왼눈 꽃 파티클
    _fc = [(255,150,200),(255,220,80),(200,150,255),(255,180,120)]
    for _fi in range(4):
        _fa = t*1.2+_fi*1.57
        _fr = bw*(0.55+math.sin(t*0.5+_fi)*0.15)
        _fx = int(ex_l-abs(math.cos(_fa))*_fr*0.7)
        _fy = int(ey+math.sin(_fa)*bh*0.45)
        _ffl = 0.4+0.6*abs(math.sin(t*2+_fi*1.3))
        if _ffl<0.2: continue
        _fsz = max(2,int(4*_ffl))
        for _pi2 in range(5):
            _pa = _pi2*(math.pi*2/5)+t*0.5
            pygame.draw.circle(surf,(*_fc[_fi%4],int(200*_ffl)),
                               (int(_fx+math.cos(_pa)*_fsz*1.3),
                                int(_fy+math.sin(_pa)*_fsz*1.3)),max(1,_fsz//2))
        pygame.draw.circle(surf,(255,255,200,int(220*_ffl)),(_fx,_fy),max(1,_fsz//3))
...
def _draw_sprout(surf, cx, top_y, bw, bh):
    ...
    sw = max(2, bw//10)
    stem_h = int(bh * 0.30)
    stem_top = top_y - stem_h

    # 줄기
    pygame.draw.rect(surf, sc_d, (cx - sw//2, stem_top, sw, stem_h))

    lw = max(6, int(bw * 0.28))
    lh = min(lw - 2, max(5, int(bh * 0.34)))  # lh < lw 항상 보장 → 타원 유지

    # 왼쪽 잎
    lx = cx - lw - sw//2 + max(2, sw//2)
    ly = stem_top - int(lh * 0.65)
    pygame.draw.ellipse(surf, sc,   (lx, ly, lw, lh))
    pygame.draw.ellipse(surf, sc_d, (lx, ly, lw, lh), max(1, sw//3))
    pygame.draw.ellipse(surf, sc_h, (lx + lw//4, ly + lh//5, lw//2, lh//3))

    # 오른쪽 잎
    rx = cx + sw//2 - max(2, sw//2)
    ry = stem_top - int(lh * 0.65)
    pygame.draw.ellipse(surf, sc,   (rx, ry, lw, lh))
    pygame.draw.ellipse(surf, sc_d, (rx, ry, lw, lh), max(1, sw//3))
    pygame.draw.ellipse(surf, sc_h, (rx + lw//4, ry + lh//5, lw//2, lh//3))

def _draw_demon_wings(surf, cx, cy, bw, bh, frozen=False, bright=False, wing_color=None):
    """악마 해파리 날개 — 황금해파리처럼 타원 글로우 + 파닥임."""
    t      = pygame.time.get_ticks() * 0.001
    if frozen:
        flap_y = 0; flap_h = 1.0
    else:
        _s     = math.sin(t * 7.5)
        flap_y = int(bh * 0.15 * _s)
        flap_h = 1.0 + 0.20 * abs(_s)
    wc     = wing_color if wing_color else (18, 18, 22)
    gc     = (100, 20, 200)
    # ① 배경 타원 글로우 — 흰 날개면 흰 글로우, 아니면 붉은 글로우
    pg  = 0.6 + abs(math.sin(t * 2.0)) * 0.4
    rc  = (200, 200, 200) if wing_color else (220, 40, 40)
    for ring in range(4, 0, -1):
        if wing_color:
            grw = int(bw * (1.40 + ring * 0.06 * pg))
            grh = int(bh * (0.75 + ring * 0.04 * pg))
        else:
            grw = int(bw * (1.80 + ring * 0.12 * pg))
            grh = int(bh * (1.05 + ring * 0.08 * pg))
        ga  = int(min(255, 15 * pg * ring // 4))
        gs  = pygame.Surface((grw+4, grh+4), pygame.SRCALPHA)
        gs.fill((0,0,0,0))
        pygame.draw.ellipse(gs, (*rc, ga), (2, 2, grw, grh))
        surf.blit(gs, (cx - grw//2 - 2, cy + flap_y - grh//2 - 2))
    ww  = int(bw * 0.42); wh = int(bh * 0.28 * flap_h)
    wy  = cy - int(bh * 0.05) + flap_y + 3
    for sd in (-1, 1):
        bx = cx + sd * (bw // 2 - (max(6, bw//10) if bright else max(10, bw//6)))
        pts = [
            (bx,                      wy - int(wh*0.45)),
            (bx + sd*int(ww*0.38),    wy - wh),
            (bx + sd*ww,              wy - int(wh*0.25)),
            (bx + sd*int(ww*0.80),    wy + int(wh*0.10)),
            (bx + sd*int(ww*0.62),    wy + int(wh*0.35)),
            (bx + sd*int(ww*0.45),    wy + int(wh*0.12)),
            (bx + sd*int(ww*0.28),    wy + int(wh*0.35)),
            (bx,                      wy + int(wh*0.35)),
        ]
        # ② 날개 외곽 bloom — wing_color 있으면 흰색, 없으면 붉은색
        _xs=[p[0] for p in pts]; _ys=[p[1] for p in pts]
        _pulse = 0.65 + 0.35 * abs(math.sin(t * 3.0))
        _al_mult = 2.2 if bright else 1.0
        for _g in range(7, 0, -1):
            _lw = _g * 2
            _brightness = int(255 * (1 - _g/8))
            _al = int(min(255, 50 * _pulse * (8-_g) // 7 * _al_mult))
            if wing_color:
                _col = (min(255,_brightness+180), min(255,_brightness+180), min(255,_brightness+180))
            else:
                _col = (min(255,_brightness+180), max(0,_brightness-80), max(0,_brightness-80))
            _pad = _lw + 2
            _sw2=max(1,max(_xs)-min(_xs)+_pad*2); _sh3=max(1,max(_ys)-min(_ys)+_pad*2)
            _ox2=min(_xs)-_pad; _oy2=min(_ys)-_pad
            _gse=pygame.Surface((_sw2,_sh3),pygame.SRCALPHA)
            _gse.fill((0,0,0,0))
            _lpe=[(p[0]-_ox2,p[1]-_oy2) for p in pts]
            pygame.draw.polygon(_gse,(*_col,_al),_lpe,_lw)
            surf.blit(_gse,(_ox2,_oy2))
...
def _draw_online_action(surf, cx, cy, bw, bh, action, phase, label_top_y=None):
    ...
    elif action == 'thumbsup2':
        # 쌍따봉: 따봉의 양팔 버전
        bob = int(math.sin(phase * 3.2) * bh * 0.035)
        for side in (1, -1):
            s1x = cx + side*int(bw*0.40); s1y = cy + bh//8
            e1x = cx + side*int(bw*0.66); e1y = cy + int(bh*0.04) + bob
            pygame.draw.line(surf, (*tc,215), (s1x,s1y), (e1x,e1y), thick)
            e2x = cx + side*int(bw*0.80); e2y = cy - int(bh*0.26) + bob
            pygame.draw.line(surf, (*tc,215), (e1x,e1y), (e2x,e2y), thick)
            fist_r = max(3, bw//11)
            pygame.draw.circle(surf, tc, (e2x, e2y), fist_r)
            tbx = e2x + side*int(bw*0.03); tby = e2y - int(bh*0.22) + bob
            pygame.draw.line(surf, (*tc,225), (e2x, e2y-fist_r+1), (tbx,tby), max(2,thick-1))
            pygame.draw.circle(surf, (*tc,210), (tbx,tby), max(2,bw//20))
    elif action == 'smoking':
...
def draw_settings_screen(surf, bgm_vol, sfx_vol, chat_vol=0.7, always_on_top=False, page=0, shortcut_catch=None, shortcut_kill=None, binding_target=None):
    ...
    if page == 0:
        ...
        tp=pct.render(f'{int(bgm_vol*100)}%',True,(220,235,255))
        ...
        surf.blit(_aot_label, (SL_BGM[0], SL_BGM[1]-28))
        _aot_bx = SL_BGM[0]+SL_BGM[2]-44; _aot_by = SL_BGM[1]-30
...
def draw_nickname_input(surf, text, cursor_on, changing=False):
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    overlay.fill((5, 10, 30, 245))
    surf.blit(overlay, (0,0))
    ft = get_font(20, bold=True)
    title = '닉네임 변경' if changing else '닉네임을 입력해주세요'
    nt = ft.render(title, True, (200,230,255))
    surf.blit(nt, (WIDTH//2-nt.get_width()//2, HEIGHT//2-80))
    fd = get_font(12)
    hint = 'ESC로 취소 · 최대 12자' if changing else '랭킹에 표시될 이름이에요 (최대 12자)'
    dt = fd.render(hint, True, (120,155,195))
...
def draw_ranking_screen(surf, rankings, loading, nickname=''):
    ...
    # 닉네임 변경 버튼
    nb = pygame.Surface((RANKING_NICK_BTN.w, RANKING_NICK_BTN.h), pygame.SRCALPHA)
    nb.fill((22,42,88,210))
    pygame.draw.rect(nb, (75,135,215), (0,0,RANKING_NICK_BTN.w,RANKING_NICK_BTN.h), 1, border_radius=5)
    surf.blit(nb, (RANKING_NICK_BTN.x, RANKING_NICK_BTN.y))
    fn_nb = get_font(11, bold=True)
    tn_nb = fn_nb.render('닉네임 변경', True, (165,210,255))
    surf.blit(tn_nb, (RANKING_NICK_BTN.x+RANKING_NICK_BTN.w//2-tn_nb.get_width()//2,
                      RANKING_NICK_BTN.y+RANKING_NICK_BTN.h//2-tn_nb.get_height()//2))
    if loading:
...
def draw_cult_doc_list(surf, cult_docs, scroll_y=0):
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    ...
    # 클립 영역 (헤더/푸터 제외)
    CLIP_TOP = 54; CLIP_BOT = HEIGHT - 32
    clip_surf = pygame.Surface((WIDTH, CLIP_BOT - CLIP_TOP), pygame.SRCALPHA)

    cy_off = -scroll_y
    has_any = False
    total_h = 0
    for doc_type in sorted(cult_docs):
        if doc_type not in CULT_DOC_NAMES: continue
        ...
        if -card_h < y < CLIP_BOT - CLIP_TOP:
            ...
            clip_surf.blit(card, (18, y))

            sx, sy = 30, y+10
            pygame.draw.rect(clip_surf, (242,235,210), (sx, sy, 26, 36), border_radius=3)
            pygame.draw.rect(clip_surf, (188,165,118), (sx, sy, 26, 36), 1, border_radius=3)
            pygame.draw.polygon(clip_surf, (210,198,168), [(sx+19,sy),(sx+26,sy+7),(sx+26,sy)])
            pygame.draw.line(clip_surf,(188,165,118),(sx+19,sy),(sx+26,sy+7),1)
            for i in range(3):
                pygame.draw.line(clip_surf,(168,145,100),(sx+4,sy+9+i*7),(sx+22,sy+9+i*7),1)

            fn = get_font(14, bold=True)
            nt = fn.render(name, True, (195, 242, 205))
            clip_surf.blit(nt, (sx+34, y+8))
            fd = get_font(11)
            dt = fd.render(desc, True, (128, 175, 142))
            clip_surf.blit(dt, (sx+34, y+26))
            ...
            if result_slot is not None:
                ...
                bg_g.blit(gl_t,(5,2)); clip_surf.blit(bg_g,(sx+34+rt.get_width()+6, y+44))
                bi2 = _slot_base_idx(result_slot)
                mspr = (RAINBOW_BELL_SPRITE if result_slot==22
                        ...
                        else DEMON_BELL_SPRITE if result_slot==29
                        else CHAOS_BELL_SPRITE  if result_slot==30
                        else SPROUT_BELL_SPRITE if result_slot==31
                        else FAKE_JELLY_SPRITE  if result_slot==32
                        else HNA_BELL_SPRITE    if result_slot==36
                        else BELL_SPRITES[bi2])
                mspr = pygame.transform.scale(mspr, (34, 26))
                _ms_x = WIDTH-56; _ms_y = y+card_h//2-13
                if result_slot == 29: _draw_demon_wings(clip_surf, _ms_x+17, _ms_y+13, 34, 26)
                if result_slot == 30: _draw_demon_wings(clip_surf, _ms_x+17, _ms_y+13, 34, 26, wing_color=(240,240,240))
                if result_slot == 33:
                    _draw_hologram_effect(clip_surf, _ms_x+17, _ms_y+13, 34, 26, mspr)
                else:
                    clip_surf.blit(mspr, (_ms_x, _ms_y))
                if result_slot == 30:
                    _t_c30 = pygame.time.get_ticks()*0.001
                    _scx30 = _ms_x+17; _scy30 = _ms_y+13
                    for _ic30 in range(3):
                        _a_c30 = _t_c30*0.9+_ic30*(math.pi*2/3)
                        _ox_c30 = _scx30+int(math.cos(_a_c30)*34*0.65)
                        _oy_c30 = _scy30+int(math.sin(_a_c30)*26*0.55)
                        _fl_c30 = 0.6+abs(math.sin(_t_c30*7+_ic30*2))*0.4
                        _r_c30 = max(1,int((1+abs(math.sin(_t_c30*4+_ic30)))*_fl_c30))
                        pygame.draw.circle(clip_surf,(230,230,230),(_ox_c30,_oy_c30),_r_c30)
                _draw_slot_overlay(clip_surf, result_slot, _ms_x+17, _ms_y+4, 34, 26)

            fc2 = get_font(15, bold=True)
            ct = fc2.render(f'× {count}', True, (158, 255, 168))
            clip_surf.blit(ct, (WIDTH-28-ct.get_width(), y+8))

        cy_off += card_h + 8
        total_h += card_h + 8

    surf.blit(clip_surf, (0, CLIP_TOP))

    # 스크롤바
    if total_h > CLIP_BOT - CLIP_TOP:
        _tr_x = WIDTH-5; _tr_y = CLIP_TOP; _tr_h = CLIP_BOT - CLIP_TOP
        _max_s = total_h - (CLIP_BOT - CLIP_TOP)
        pygame.draw.rect(surf, (20,60,35), (_tr_x, _tr_y, 3, _tr_h), border_radius=2)
        _th = max(20, int(_tr_h * (CLIP_BOT-CLIP_TOP) / total_h))
        _tp = int((_tr_h-_th) * scroll_y / _max_s) if _max_s else 0
        pygame.draw.rect(surf, (80,180,120), (_tr_x, _tr_y+_tp, 3, _th), border_radius=2)
...
def draw_doc_detail(surf, doc_type, inventory):
    ...
    if recipe and result_slot is not None:
        ...
        def draw_jelly_card(cx, cy, slot):
            ...
            if slot == 29: _draw_demon_wings(surf, cx, cy, sw, sh)
            if slot == 30: _draw_demon_wings(surf, cx, cy, sw, sh, wing_color=(240,240,240))
            surf.blit(spr, (cx-sw//2, cy-sh//2))
            if slot == 30:
                _t_dd30 = pygame.time.get_ticks()*0.001
                _spy_dd30 = cy
                for _idd30 in range(3):
                    _a_dd30 = _t_dd30*0.9+_idd30*(math.pi*2/3)
                    _ox_dd30 = cx+int(math.cos(_a_dd30)*sw*0.65)
                    _oy_dd30 = _spy_dd30+int(math.sin(_a_dd30)*sh*0.55)
                    _fl_dd30 = 0.6+abs(math.sin(_t_dd30*7+_idd30*2))*0.4
                    _r_dd30 = max(2,int((2+abs(math.sin(_t_dd30*4+_idd30))*2)*_fl_dd30))
                    _gs_dd30 = pygame.Surface((_r_dd30*4+2,_r_dd30*4+2),pygame.SRCALPHA)
                    pygame.draw.circle(_gs_dd30,(220,220,220,int(30*_fl_dd30)),(_r_dd30*2+1,_r_dd30*2+1),_r_dd30*2)
                    surf.blit(_gs_dd30,(_ox_dd30-_r_dd30*2-1,_oy_dd30-_r_dd30*2-1))
                    pygame.draw.circle(surf,(230,230,230),(_ox_dd30,_oy_dd30),_r_dd30)
...
def draw_gacha_screen(surf, slot, timer):
    ...
    if jp > 0:
        ...
        if slot == 30: _draw_demon_wings(surf, cx2, cy2+sh2//6, sw2, sw2//2, wing_color=(240,240,240))
        if slot == 33:
            _draw_hologram_effect(surf, cx2, cy2, sw2, sh2, spr2)
        else:
            surf.blit(spr2, (cx2-sw2//2, cy2-sh2//2))
        if slot == 30:
            _t_g30 = pygame.time.get_ticks()*0.001
            _spy_g30 = cy2+sh2//2
            for _ig30 in range(3):
                _a_g30 = _t_g30*0.9+_ig30*(math.pi*2/3)
                _ox_g30 = cx2+int(math.cos(_a_g30)*sw2*0.65)
                _oy_g30 = _spy_g30+int(math.sin(_a_g30)*sh2*0.55)
                _fl_g30 = 0.6+abs(math.sin(_t_g30*7+_ig30*2))*0.4
                _r_g30 = max(2,int((2+abs(math.sin(_t_g30*4+_ig30))*2)*_fl_g30))
                _gs_g30 = pygame.Surface((_r_g30*4+2,_r_g30*4+2),pygame.SRCALPHA)
                pygame.draw.circle(_gs_g30,(220,220,220,int(30*_fl_g30)),(_r_g30*2+1,_r_g30*2+1),_r_g30*2)
                surf.blit(_gs_g30,(_ox_g30-_r_g30*2-1,_oy_g30-_r_g30*2-1))
                pygame.draw.circle(surf,(230,230,230),(_ox_g30,_oy_g30),_r_g30)
...
def _slot_base_idx(slot):
    return {0:0,1:1,2:0,3:0,4:2,5:0,6:3,7:4,8:5,9:6,10:7,11:8,12:9,13:10,14:11,15:12,16:13,17:14,18:15,19:16,20:17,21:18,22:23,23:24,24:19,25:20,26:21,27:22,28:0,29:0}.get(slot,0)
...
def _draw_slot_overlay(surf, slot, pcx, pcy, sw, sh, happy=False):
    ...
    # slot 15 (구름 해파리)는 기본 스프라이트 그대로 표시
    ...
    elif slot == 29:  # 악마 해파리: 발광 눈 + 악마 날개
        t29   = pygame.time.get_ticks() * 0.001
        pulse = 0.7 + abs(math.sin(t29 * 2.8)) * 0.3
        ec29  = (252, 27, 27)
        ey29  = pcy + int(sh * 0.58)
        # ── 눈: 화난 해파리 눈썹과 동일한 방식 (\  /) ──────────
        _ox29=sw//9; _th29=max(1,sw//16); _ec29_e=(220,20,20)
        pygame.draw.line(surf,_ec29_e,(pcx-sw//6-_ox29//2,ey29-sh//18),(pcx-sw//6+_ox29//2,ey29+sh//18),_th29)
        pygame.draw.line(surf,_ec29_e,(pcx+sw//7-_ox29//2,ey29+sh//18),(pcx+sw//7+_ox29//2,ey29-sh//18),_th29)
        # 날개는 벨 이전에 별도 호출됨 (_draw_slot_overlay에서 제외)
    elif slot == 30:  # 혼돈 해파리: 헤일로 + 혼돈 눈 (파랑 왼쪽, 빨강 오른쪽)
        # 흰색 헤일로
        hlw=int(sw*0.80); hlh=max(1,int(hlw*3//11))
        _hls30 = pygame.transform.scale(HALO_BASE,(hlw,hlh))
        _wh30 = pygame.Surface((hlw,hlh),pygame.SRCALPHA)
        _mk30h = pygame.mask.from_surface(_hls30)
        _wh30.blit(_mk30h.to_surface(setcolor=(255,255,255,255),unsetcolor=(0,0,0,0)),(0,0))
        surf.blit(_wh30, (pcx-hlw//2, pcy-4-hlh-2))
        # 여우불 오브 3개
        _t30 = pygame.time.get_ticks() * 0.001
        for _i30 in range(3):
            _a30 = _t30*0.9 + _i30*(math.pi*2/3)
            _ox30 = pcx + int(math.cos(_a30)*sw*0.65)
            _oy30 = (pcy+sh//2-4) + int(math.sin(_a30)*sh*0.55)
            _fl30 = 0.6 + abs(math.sin(_t30*7+_i30*2))*0.4
            _r30 = max(2, int((2+abs(math.sin(_t30*4+_i30))*2)*_fl30))
            _gs30 = pygame.Surface((_r30*4+2, _r30*4+2), pygame.SRCALPHA)
            pygame.draw.circle(_gs30, (220,220,220,int(30*_fl30)), (_r30*2+1,_r30*2+1), _r30*2)
            surf.blit(_gs30, (_ox30-_r30*2-1, _oy30-_r30*2-1))
            pygame.draw.circle(surf, (230,230,230), (_ox30,_oy30), _r30)
        # 픽셀 도트 눈
        _ey30 = (pcy+sh//2-4) + int(sh*0.08)
        _pw30 = max(3, sw//12); _ph30 = max(3, sh//7)
        pygame.draw.rect(surf, (20,80,255),  (pcx-sw//8,  _ey30, _pw30, _ph30))
    ...
    elif slot == 36:  # 희노애락 해파리
        _draw_hna_eyes(surf, pcx, pcy + sh//2 - 4, sw, sh)
        ...
        _t34 = pygame.time.get_ticks() * 0.001
        _cy34 = pcy + sh//2 - 4
        _er34 = max(3, sw//9)
        _ex_l34 = pcx - int(sw*0.17); _ex_r34 = pcx + int(sw*0.15)
        _ey34 = _cy34 + sh//8
        # 눈
        for _ex34 in (_ex_l34, _ex_r34):
            pygame.draw.circle(surf, (245,250,255), (_ex34,_ey34), _er34)
            pygame.draw.circle(surf, (15,30,70), (_ex34,_ey34), max(2,_er34-1))
            pygame.draw.circle(surf, (255,255,255), (_ex34-_er34//3,_ey34-_er34//3), max(1,_er34//3))
            pygame.draw.circle(surf, (255,255,255), (_ex34+_er34//4,_ey34-_er34//4), max(1,_er34//5))
        # 눈썹 (눈 위에 덮어그림)
        _eby34 = _ey34 - _er34 - max(1,sh//14)
        _ox34 = sw//6; _th34 = max(1,sw//11)
        pygame.draw.line(surf,(20,20,20),(_ex_l34-_ox34//2,_eby34+sh//16),(_ex_l34+_ox34//2,_eby34-sh//16),_th34)
        pygame.draw.line(surf,(20,20,20),(_ex_r34-_ox34//2,_eby34-sh//16),(_ex_r34+_ox34//2,_eby34+sh//16),_th34)
        # 눈물 (작게)
        for _ex34t in (_ex_l34, _ex_r34):
            for _di in range(2):
                _td = (_t34*1.5 + _di*0.8) % 2.0
                if _td > 1.2: continue
                _dy34 = _ey34 + sh//6 + int(_td*sh*0.45)
                _da = int(180*(1-_td/1.2)); _dr = max(1,sw//16)
                _ts34 = pygame.Surface((_dr*2+2,_dr*3+2),pygame.SRCALPHA)
                pygame.draw.ellipse(_ts34,(235,243,254,_da),(1,_dr//2,_dr*2,_dr*2))
                surf.blit(_ts34,(_ex34t-_dr-1,_dy34-_dr//2))
    elif slot == 28:  # 아기 해파리: 쪽쪽이 + 우유병
        cx28 = pcx; cy28 = pcy + sh // 2 - 4
        # 쪽쪽이 (눈 사이 아래, 노란색)
        gr28 = max(3, sw // 10)
        py28 = cy28 + sh // 7 + gr28 // 2 + 1
        pygame.draw.ellipse(surf, (255, 222, 60), (cx28 - gr28, py28, gr28 * 2, int(gr28 * 1.3)))
        pygame.draw.ellipse(surf, (210, 175, 20), (cx28 - gr28, py28, gr28 * 2, int(gr28 * 1.3)), max(1, gr28 // 4))
        pygame.draw.circle(surf, (240, 200, 40), (cx28, py28 + int(gr28 * 0.65)), max(1, gr28 // 3))
        nip28 = max(3, gr28 // 2)
        pygame.draw.rect(surf, (255, 235, 130), (cx28 - max(2, gr28 // 4), py28 + int(gr28 * 1.3) - 1, max(4, gr28 // 2), nip28), border_radius=1)
        # 우유병 (왼쪽 아래)
        t28   = pygame.time.get_ticks() * 0.001
        bob28 = int(math.sin(t28 * 1.8) * 1.5)
        bw28  = max(5, sw // 7); bh28 = max(8, sw // 4); cap28 = max(3, sw // 10)
        hx28  = cx28 - int(sw * 0.42) + bob28
        hy28  = cy28 + sh // 2 - bh28 - cap28 + 2
        pygame.draw.line(surf, (222, 158, 175), (cx28 - int(sw * 0.28), cy28 + sh // 2 - 4), (hx28 + bw28 // 2, hy28 + bh28 + cap28), 2)
        pygame.draw.rect(surf, (255, 210, 60), (hx28, hy28, bw28, cap28), border_radius=1)
        pygame.draw.rect(surf, (220, 170, 20), (hx28, hy28, bw28, cap28), 1, border_radius=1)
        pygame.draw.rect(surf, (245, 245, 250), (hx28, hy28 + cap28, bw28, bh28), border_radius=2)
        pygame.draw.rect(surf, (190, 190, 205), (hx28, hy28 + cap28, bw28, bh28), 1, border_radius=2)
        pygame.draw.line(surf, (185, 220, 255), (hx28 + 2, hy28 + cap28 + bh28 // 3), (hx28 + bw28 - 3, hy28 + cap28 + bh28 // 3), 1)
        nip_h28 = max(2, sw // 12)
        pygame.draw.rect(surf, (245, 245, 250), (hx28 + bw28 // 2 - 1, hy28 + cap28 + bh28, 2, nip_h28), border_radius=1)
...
def draw_inventory(surf, inventory, page=0):
    ...
    for pos_i, (cx, cy) in enumerate(positions):
        ...
        if count == 0 and not discovered:
            ...
            elif slot == 30: spr = pygame.transform.scale(CHAOS_BELL_SPRITE, (sw, sh))
            elif slot == 31: spr = pygame.transform.scale(SPROUT_BELL_SPRITE, (sw, sh))
            ...
            elif slot == 36: spr = pygame.transform.scale(HNA_BELL_SPRITE, (sw, sh))
            ...
            if slot == 33:
                _draw_hologram_effect(surf, pcx, pcy+sh//2-4, sw, sh, spr)
            else:
                surf.blit(spr, (pcx-sw//2, pcy-4))
            if slot == 30:
                _t_i30 = pygame.time.get_ticks()*0.001
                _spy_i30 = pcy+sh//2-4
                for _ii30 in range(3):
                    _a_i30 = _t_i30*0.9+_ii30*(math.pi*2/3)
                    _ox_i30 = pcx+int(math.cos(_a_i30)*sw*0.65)
                    _oy_i30 = _spy_i30+int(math.sin(_a_i30)*sh*0.55)
                    _fl_i30 = 0.6+abs(math.sin(_t_i30*7+_ii30*2))*0.4
                    _r_i30 = max(2,int((2+abs(math.sin(_t_i30*4+_ii30))*2)*_fl_i30))
                    _gs_i30 = pygame.Surface((_r_i30*4+2,_r_i30*4+2),pygame.SRCALPHA)
                    pygame.draw.circle(_gs_i30,(220,220,220,int(30*_fl_i30)),(_r_i30*2+1,_r_i30*2+1),_r_i30*2)
                    surf.blit(_gs_i30,(_ox_i30-_r_i30*2-1,_oy_i30-_r_i30*2-1))
                    pygame.draw.circle(surf,(230,230,230),(_ox_i30,_oy_i30),_r_i30)
...
def draw_jelly_detail(surf, slot, inventory):
    ...
    if count > 0 or discovered:
        ...
        if slot == 33:
            ...
            surf.blit(spr, (pcx - sw//2, pcy - 4))
        if slot == 30:
            _t_d30 = pygame.time.get_ticks()*0.001
            _spy_d30 = pcy+sh//2-4
            for _id30 in range(3):
                _a_d30 = _t_d30*0.9+_id30*(math.pi*2/3)
                _ox_d30 = pcx+int(math.cos(_a_d30)*sw*0.65)
                _oy_d30 = _spy_d30+int(math.sin(_a_d30)*sh*0.55)
                _fl_d30 = 0.6+abs(math.sin(_t_d30*7+_id30*2))*0.4
                _r_d30 = max(2,int((2+abs(math.sin(_t_d30*4+_id30))*2)*_fl_d30))
                _gs_d30 = pygame.Surface((_r_d30*4+2,_r_d30*4+2),pygame.SRCALPHA)
                pygame.draw.circle(_gs_d30,(220,220,220,int(30*_fl_d30)),(_r_d30*2+1,_r_d30*2+1),_r_d30*2)
                surf.blit(_gs_d30,(_ox_d30-_r_d30*2-1,_oy_d30-_r_d30*2-1))
                pygame.draw.circle(surf,(230,230,230),(_ox_d30,_oy_d30),_r_d30)
        _draw_slot_overlay(surf, slot, pcx, pcy, sw, sh)
    else:
        dark = spr.copy()
...
class Jellyfish:
    ...
    def _apply_type(self):
        ...
        self.is_demon  = False
        self.is_chaos    = False
        self.is_sprout   = False
        self.is_fakejelly  = False
        self.is_amethyst   = False
        ...
        # DEV 스폰 필터 우선 적용
        if DEV_MODE and _dev_spawn_idx is not None:
            self._apply_forced(_dev_spawn_idx)
            return
        ...
        if not DEV_MODE:
            ...
            elif chosen == 30: base_idx = 5;  self.is_chaos   = True
            elif chosen == 31: base_idx = 0;  self.is_sprout   = True
            ...
            elif chosen == 36: base_idx = 0;  self.is_hna        = True
            else:              base_idx = 0
        ...
        elif self.design_idx == 28:
            self.bell_sprite = BABY_BELL_SPRITE
        elif self.design_idx == 29:
            self.bell_sprite = DEMON_BELL_SPRITE
        elif self.design_idx == 30:
            self.bell_sprite = CHAOS_BELL_SPRITE
        elif self.design_idx == 31:
            self.bell_sprite = SPROUT_BELL_SPRITE
        elif self.design_idx == 32:
            self.bell_sprite = FAKE_JELLY_SPRITE
        ...
        elif self.design_idx == 36:
            self.bell_sprite = HNA_BELL_SPRITE
        ...
        elif self.design_idx == 36: self.body_color = (180, 215, 100)
        ...
        if self.is_fakejelly:
            self.bw0 = max(36, int(W_PIX*defn['ps']*sf*1.15))
            self.bh0 = max(12, int(self.BH*defn['ps']*sf*0.55))
    ...
    def _apply_forced(self, idx):
        """DEV 스폰 필터: 특정 design_idx로 강제 초기화."""
        # 기존 _apply_type 로직과 동일하게 chosen=idx로 처리
        self.design_idx = idx
        # chosen에 맞는 base_idx/flag 설정 (dict 방식)
        _map = {
            0:(0,{}),1:(1,{}),2:(0,{'has_hat':True}),3:(0,{'has_glasses':True}),
            4:(2,{'is_electric':True}),5:(0,{'has_cat':True}),6:(3,{'is_slime':True}),
            7:(4,{'is_frozen':True}),8:(5,{'is_angel':True}),9:(6,{'is_ghost':True}),
            10:(7,{'is_hairy':True}),11:(8,{'is_king':True}),12:(9,{'is_zombie':True}),
            13:(10,{'is_deep':True}),14:(11,{'is_cloud':True}),15:(12,{'is_angry':True}),
            16:(13,{'is_dapper':True}),17:(14,{'is_rabbit':True}),18:(15,{'is_demon':True}),
            19:(16,{'is_cactus':True}),20:(17,{'is_snowman':True}),21:(18,{'is_golden':True}),
            22:(23,{'is_rainbow':True}),23:(24,{}),24:(19,{'is_pudding':True}),
            25:(20,{'is_pudding':True}),26:(21,{'is_sakura':True}),
            27:(22,{'is_twin':True}),28:(0,{}),29:(0,{}),
            30:(5,{'is_chaos':True}),
            31:(0,{'is_sprout':True}),
        ...
        sf = 1.0
        self.bw0 = max(16, int(W_PIX*defn['ps']*sf))
        self.bh0 = max(8,  int(len(defn['art'])*defn['ps']*sf))
        if   idx==28: self.bell_sprite=BABY_BELL_SPRITE
        elif idx==29: self.bell_sprite=DEMON_BELL_SPRITE
        elif idx==30: self.bell_sprite=CHAOS_BELL_SPRITE
        elif idx==31: self.bell_sprite=SPROUT_BELL_SPRITE
        ...
        elif idx==36: self.bell_sprite=HNA_BELL_SPRITE
        ...
        else: self.bell_sprite=BELL_SPRITES[base_idx]
        if idx==28: self.tc=(222,158,175)
        elif idx==29: self.tc=(90,15,170)
        elif idx==30: self.tc=(180,100,255)
        elif idx==31: self.tc=(201,230,105)
        ...
        elif idx==36: self.tc=(105,205,162)
        else: self.tc=defn['tc']
        self.tb=defn['tb']
        self.BH=len(defn['art'])
        self.body_color=defn['cmap'].get('M',self.tc)
        if self.design_idx == 31: self.body_color=(201,230,105)
        elif self.design_idx == 32: self.body_color=(140,175,195)
        elif self.design_idx == 34: self.body_color=(56,134,244)
        elif self.design_idx == 35: self.body_color=(160,100,35)
        elif self.design_idx == 36: self.body_color=(150,100,200)
    ...
    def draw(self, surf):
        ...
        # 악마 날개 — 벨 직전에 그려서 얼굴 뒤에 위치
        if self.design_idx == 29:
            _draw_demon_wings(surf, x, y, bw, bh, frozen=self.is_dead)
        if self.design_idx == 30:
            _draw_demon_wings(surf, x, y, bw, bh, frozen=self.is_dead, wing_color=(240,240,240))

        # 자수정: 후광을 벨 앞에 그리기 (벨 뒤 레이어)
        if self.is_amethyst:
            t_am = self.amethyst_phase
            gr2 = int(60  + 195 * (0.5 + 0.5 * math.sin(t_am * 0.7 + 0.5)))
            gg2 = int(215 * (0.5 + 0.5 * math.sin(t_am * 0.5 + 2.5)))
            gb2 = int(80  + 175 * (0.5 + 0.5 * math.sin(t_am * 0.6 + 4.5)))
            for _gr in range(3, 0, -1):
                _ga = int(22 + 10 * abs(math.sin(t_am * 0.9 + _gr)))
                _gs_am = pygame.Surface((bw + _gr*6, bh + _gr*4), pygame.SRCALPHA)
                pygame.draw.ellipse(_gs_am, (gr2, gg2, gb2, _ga),
                                    (0, 0, bw+_gr*6, bh+_gr*4))
                surf.blit(_gs_am, (x-bw//2-_gr*3, y-bh//2-_gr*2))
        ...
        # 자수정: 홀로그래픽 줄무늬가 아래로 흘러내림
        if self.is_amethyst:
            t_am = self.amethyst_phase
            _sw2, _sh2 = bell_s.get_size()
            _stripe_surf = pygame.Surface((_sw2, _sh2), pygame.SRCALPHA)
            # 홀로그래픽 색상 팔레트 (애니 머리카락처럼)
            _holo = [
                (0,   220, 220),   # 청록
                (80,  255, 190),   # 민트
                (200, 130, 255),   # 라벤더
            ...
            _n = len(_holo)
            _sh_stripe = max(4, _sh2 // _n)
            _offset = int((t_am * 18) % _sh2)
            for _ci, _hc in enumerate(_holo):
                _sy = (_ci * _sh_stripe - _offset) % _sh2
                # 줄무늬 두 개 그려서 화면 끝에서도 이어지게
                for _dy in (_sy, _sy - _sh2):
                    if -_sh_stripe < _dy < _sh2:
                        pygame.draw.rect(_stripe_surf, (*_hc, 75),
                                         (0, _dy, _sw2, _sh_stripe * 3 // 5))
            bell_s.blit(_stripe_surf, (0, 0), special_flags=pygame.BLEND_RGB_ADD)
        ...
        if self.is_amethyst:
            ...
            for _i in range(8):
                _seed = _i * 137.5
                _px2 = x + int(math.cos(_seed) * bw * (0.5 + (_i % 3) * 0.15))
                _py2 = y + int(math.sin(_seed * 0.7) * bh * (0.4 + (_i % 4) * 0.12))
                _fl2 = 0.5 + 0.5 * math.sin(t_am * (3.0 + _i * 0.7) + _i * 2.3)
                if _fl2 < 0.1: continue
                _sz2 = max(3, int(5 * _fl2))
                _pr = int(150 + 105 * (0.5 + 0.5 * math.sin(t_am * 0.8 + _i)))
                _pg = int(200 * (0.5 + 0.5 * math.sin(t_am * 0.55 + _i * 1.3)))
                _pb = int(180 + 75 * (0.5 + 0.5 * math.sin(t_am * 0.65 + _i * 0.9)))
                _al2 = int(220 * _fl2)
                _sp_s = pygame.Surface((_sz2*2+4, _sz2*2+4), pygame.SRCALPHA)
                _c2 = _sz2 + 2
                pygame.draw.line(_sp_s, (_pr, _pg, _pb, _al2),
                                 (_c2-_sz2, _c2), (_c2+_sz2, _c2), max(2, _sz2//2))
                pygame.draw.line(_sp_s, (_pr, _pg, _pb, _al2),
                                 (_c2, _c2-_sz2), (_c2, _c2+_sz2), max(2, _sz2//2))
                _s2 = max(1, _sz2//2)
                pygame.draw.line(_sp_s, (_pr, _pg, _pb, _al2//2),
                                 (_c2-_s2, _c2-_s2), (_c2+_s2, _c2+_s2), max(1,_sz2//3))
                pygame.draw.line(_sp_s, (_pr, _pg, _pb, _al2//2),
                                 (_c2+_s2, _c2-_s2), (_c2-_s2, _c2+_s2), max(1,_sz2//3))
                surf.blit(_sp_s, (_px2-_c2, _py2-_c2))
        ...
        if self.is_chaos:
            self._draw_foxfire(surf, x, y, bw, bh, fire_color=(220,220,220))
        ...
        # 혼돈 해파리 눈 — 픽셀 도트 (왼쪽 파랑, 오른쪽 빨강)
        if self.is_chaos:
            px_w = max(3, bw//12); px_h = max(3, bh//7)
            ey3 = y + bh//8
            pygame.draw.rect(surf, (20, 80, 255), (x-bw//8,  ey3, px_w, px_h))
            pygame.draw.rect(surf, (220, 20, 20), (x+bw//16, ey3, px_w, px_h))
        ...
        # 천사 링 (벨 위, 살짝 떠있음)
        if self.is_angel or self.is_chaos:
            ...
            if self.is_chaos:
                ...
                for _ring in range(5, 0, -1):
                    _grw = hlw + _ring * 4
                    _grh = hlh + _ring * 2
                    _ga = int(min(255, 22 * _flick * _ring // 5))
                    _gs = pygame.Surface((_grw+4, _grh+4), pygame.SRCALPHA)
                    pygame.draw.ellipse(_gs, (220, 220, 220, _ga), (2, 2, _grw, _grh))
                    surf.blit(_gs, (x-_grw//2-2, halo_y-_grh//4))
                _white_halo = pygame.Surface((hlw, hlh), pygame.SRCALPHA)
                _mask2 = pygame.mask.from_surface(hls)
                _ms2 = _mask2.to_surface(setcolor=(255,255,255,255), unsetcolor=(0,0,0,0))
                _white_halo.blit(_ms2, (0,0))
                surf.blit(_white_halo, (x-hlw//2, halo_y))
            else:
                surf.blit(hls,(x-hlw//2, halo_y))

        # 슬픈 해파리 — 초롱초롱 눈 + 눈물
        if self.is_sad:
            t_sad = self.sad_phase
            ey_sad = y + bh//8
            # 눈 위치 (중심 맞춤)
            ex_l = x - int(bw * 0.17)
            ex_r = x + int(bw * 0.15)
            _er = max(4, bw//9)   # 눈 반지름
            # 1. 초롱초롱 눈 먼저
            for ex_s in (ex_l, ex_r):
                pygame.draw.circle(surf, (245, 250, 255), (ex_s, ey_sad), _er)
                pygame.draw.circle(surf, (200, 225, 255), (ex_s, ey_sad), _er, 1)
                _pr = max(3, _er - 1)
                pygame.draw.circle(surf, (15, 30, 70), (ex_s, ey_sad), _pr)
                _h1r = max(1, _er//3)
                pygame.draw.circle(surf, (255, 255, 255), (ex_s - _er//3, ey_sad - _er//3), _h1r)
                _h2r = max(1, _er//5)
                pygame.draw.circle(surf, (255, 255, 255), (ex_s + _er//4, ey_sad - _er//4), _h2r)
            # 2. 눈썹은 눈 위에 덮어서 그림 (눈보다 나중에 → 위에 표시)
            _sb_col = (20, 20, 20)
            _sb_th = max(2, bw//11)
            _eby = ey_sad - _er - max(2, bh//14)  # 눈 상단에서 약간 위
            _ox = bw//6
            pygame.draw.line(surf, _sb_col,
                             (ex_l - _ox//2, _eby + bh//16),
                             (ex_l + _ox//2, _eby - bh//16), _sb_th)
            pygame.draw.line(surf, _sb_col,
                             (ex_r - _ox//2, _eby - bh//16),
                             (ex_r + _ox//2, _eby + bh//16), _sb_th)
            # 눈물 (두 눈에서 흘러내림)
            tear_col = (120, 180, 255, 200)
            for ex_t in (ex_l, ex_r):
                for drop_i in range(3):
                    # 각 물방울은 서로 다른 위상으로 흘러내림
                    t_drop = (t_sad * 1.5 + drop_i * 0.8) % 2.0
                    if t_drop > 1.2: continue
                    drop_y = ey_sad + bh//6 + int(t_drop * bh * 0.55)
                    drop_a = int(200 * (1.0 - t_drop / 1.2))
                    drop_r = max(2, int(bw//18 * (1 - t_drop * 0.4)))
                    _ts = pygame.Surface((drop_r*2+2, drop_r*3+2), pygame.SRCALPHA)
                    # 물방울: 위는 뾰족, 아래는 둥글
                    pygame.draw.ellipse(_ts, (235,243,254,drop_a),
                                        (1, drop_r//2, drop_r*2, drop_r*2+drop_r//2))
        ...
        # 희노애락 해파리 — 살아있을 때만
        if self.is_hna and not self.is_dead:
            _draw_hna_eyes(surf, x, y, bw, bh)
        # 똥 해파리 — 머리 위 똥
        if self.is_poop:
        ...
        # 새싹 해파리 — 머리 위 새싹
        if self.is_sprout:
            _bob = int(math.sin(self.pulse*1.2)*2)
            _draw_sprout(surf, x, y - bh//2 + int(bh*0.08) + _bob, bw, bh)

        # 악마 해파리 — 발광 눈 (벨 위에 그려짐)
        if self.design_idx == 29:
            _t29  = pygame.time.get_ticks() * 0.001
            _pls  = 0.7 + abs(math.sin(_t29 * 2.8)) * 0.3
            _ec   = (252, 27, 27)
            _ey   = y + int(bh * 0.08)   # 눈 y (벨 중하단)
            # 눈: 화난 해파리 눈썹과 동일 방식 (\ /) — 살아있을 때만
            if not self.is_dead:
                _ox_d=bw//9; _th_d=max(1,bw//16); _ec_d=(220,20,20)
                pygame.draw.line(surf,_ec_d,(x-bw//6-_ox_d//2,_ey-bh//18),(x-bw//6+_ox_d//2,_ey+bh//18),_th_d)
                pygame.draw.line(surf,_ec_d,(x+bw//7-_ox_d//2,_ey+bh//18),(x+bw//7+_ox_d//2,_ey-bh//18),_th_d)
        ...
        if self.is_frozen:
            ...
            # 아기 해파리: 촉수 3개 + 우유병 1개 (왼쪽 첫번째)
            self._draw_baby_tentacles(surf, x, y+bh//2, bw, bh)
        elif self.is_fakejelly:
            # 물고기 몸통 바닥에 딱 붙게 (스프라이트 7/9 지점)
            self._draw_slime_drips(surf, x, y + int(bh * 0.28), bw)
        elif self.is_hna:
            # 희노애락: 왼쪽 2개 노랑, 오른쪽 2개 청록
            amp = self._amp()
            _hna_tc = [(255,225,50),(255,225,50),(105,205,162),(105,205,162)]
            for i, dx_frac in enumerate([-0.28,-0.10,0.10,0.28]):
                ...
                col = _hna_tc[i]
                pygame.draw.line(s,(*col,195),(7,0),(7+sway,length),3)
                pygame.draw.circle(s,(*col,175),(7+sway,length),max(2,length//6))
                surf.blit(s,(bx-7,y+bh//2))
        elif self.is_chaos:
            # 혼돈: 왼쪽 2개 흰색, 오른쪽 2개 어두운 회색
            amp = self._amp()
            _chaos_cols = [(230,230,230),(230,230,230),(40,40,40),(40,40,40)]
        ...
        # 아기 해파리 쪽쪽이 + 우유병
        if self.design_idx == 28:
            self._draw_baby_extras(surf, x, y, bw, bh)
        ...
        if self.is_dead:
            ...
            xs    = max(3, bw // 12)
            cover = max(2, bw // 18)
            for _ei2, ex2 in enumerate((x - bw//7, x + bw//7)):
                if self.design_idx == 30:
                    # 혼돈: 흰 원 없음, 왼쪽=파랑X 오른쪽=빨강X
                    x_col = (20, 80, 255) if _ei2 == 0 else (220, 20, 20)
                elif self.design_idx == 29:
                    x_col = (200, 20, 20)
                else:
                    x_col = (22, 15, 15)
                if self.design_idx not in (9, 29, 30, 36):
                    pygame.draw.circle(surf, self.body_color, (ex2, ey2), cover + 2)
                pygame.draw.line(surf, x_col, (ex2-xs, ey2-xs), (ex2+xs, ey2+xs), 4)
                pygame.draw.line(surf, x_col, (ex2+xs, ey2-xs), (ex2-xs, ey2+xs), 4)
    ...
    def _draw_baby_tentacles(self, surf, cx, top_y, bw, bh):
        """아기 해파리: 기본 드립 모션 + 왼쪽 첫번째 다리에 우유병."""
        # 기존과 동일한 slime drip 모션
        self._draw_slime_drips(surf, cx, top_y, bw)
        # 왼쪽 첫번째 다리(-0.28) 위치에 우유병 오버레이
        tc = self.tc; amp = self._amp()
        bob = int(math.sin(self.tent_phase * 1.2) * amp)
        tx = cx + int(-0.28 * bw)
        bw2 = max(5, bw // 7); bh2 = max(8, bw // 4)
        cap_h = max(3, bw // 10); nip_h = max(2, bw // 12)
        arm_len = max(8, bh // 3)
        # 팔: 다리 시작점에서 옆으로 살짝 올려 들기
        hand_x = tx - int(bw * 0.14) + bob
        hand_y = top_y + arm_len // 3
        pygame.draw.line(surf, (*tc, 200), (tx, top_y + 2), (hand_x, hand_y), 3)
        # 우유병
        bot_x = hand_x - bw2 // 2; bot_y = hand_y - bh2 - cap_h
        s = pygame.Surface((bw2 + 4, bh2 + cap_h * 2 + nip_h + 4), pygame.SRCALPHA)
        pygame.draw.rect(s, (255, 210, 60), (2, 0, bw2, cap_h), border_radius=1)
        pygame.draw.rect(s, (220, 170, 20), (2, 0, bw2, cap_h), 1, border_radius=1)
        pygame.draw.rect(s, (245, 245, 250), (2, cap_h, bw2, bh2), border_radius=2)
        pygame.draw.rect(s, (190, 190, 205), (2, cap_h, bw2, bh2), 1, border_radius=2)
        mk_y = cap_h + bh2 // 3
        pygame.draw.line(s, (185, 220, 255), (4, mk_y), (bw2 - 1, mk_y), 1)
        pygame.draw.rect(s, (245, 245, 250), (2 + bw2 // 2 - 1, cap_h + bh2, 2, nip_h), border_radius=1)
        surf.blit(s, (bot_x - 2, bot_y))

    def _draw_baby_extras(self, surf, cx, cy, bw, bh):
        """아기 해파리 쪽쪽이 (눈 사이 아래, 노란색)."""
        ey = cy + bh // 7
        gr = max(3, bw // 10)
        py = ey + gr // 2 + 1
        # 가드 (노란색 타원)
        pygame.draw.ellipse(surf, (255, 222, 60), (cx - gr, py, gr * 2, int(gr * 1.3)))
        pygame.draw.ellipse(surf, (210, 175, 20), (cx - gr, py, gr * 2, int(gr * 1.3)), max(1, gr // 4))
        # 가드 가운데 버튼
        pygame.draw.circle(surf, (240, 200, 40), (cx, py + int(gr * 0.65)), max(1, gr // 3))
        # 젖꼭지 (아래로 돌출, 연한 노랑)
        nip_h = max(3, gr // 2)
        pygame.draw.rect(surf, (255, 235, 130), (cx - max(2, gr // 4), py + int(gr * 1.3) - 1, max(4, gr // 2), nip_h), border_radius=1)
    ...
    def _draw_foxfire(self, surf, cx, cy, bw, bh, fire_color=None):
        """여우불: 주변을 떠다니는 불꽃 구체. fire_color=(r,g,b)로 색 지정 가능."""
        t = self.demon_phase
        n = 5
        for i in range(n):
            ...
            for gr in range(orb_r * 4, 0, -2):
                ...
                gs = pygame.Surface((gr*2+2, gr*2+2), pygame.SRCALPHA)
                pygame.draw.circle(gs, (r, g, b, ga), (gr+1, gr+1), gr)
                surf.blit(gs, (ox-gr-1, oy-gr-1))
            ...
            for j in range(1, 4):
                ...
                ts = pygame.Surface((tr*2+2, tr*2+2), pygame.SRCALPHA)
                pygame.draw.circle(ts, (r, g, b, ta3), (tr+1, tr+1), tr)
                surf.blit(ts, (tx-tr-1, ty-tr-1))

            # 핵심 구체
            pygame.draw.circle(surf, (r, g, b), (ox, oy), orb_r)
            _cr = min(255, r+40); _cg = min(255, g+60); _cb = min(255, b+40)
            pygame.draw.circle(surf, (_cr, _cg, _cb), (ox, oy), max(1, orb_r-1))
    ...
    while running:
            ...
            if show_quit_confirm and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if binding_target and show_settings:
                    ...
                    if event.key == pygame.K_RETURN and nickname_text.strip():
                        new_nick = nickname_text.strip()[:12]
                        if nick_changing and player_nickname and player_nickname != new_nick:
                            delete_ranking_bg(player_nickname)  # 기존 닉네임 랭킹 삭제
                        player_nickname = new_nick
                        show_nickname_input = False
                        nick_changing = False
                        pygame.key.stop_text_input()
                        save_game(inventory, _current_stage, cult_docs, aquarium, player_nickname)
                        upload_score_bg(player_nickname, calc_score(inventory))
                    elif event.key == pygame.K_ESCAPE and nick_changing:
                        show_nickname_input = False
                        nick_changing = False
                        pygame.key.stop_text_input()
                    elif event.key == pygame.K_BACKSPACE:
                ...
                elif DEV_MODE and event.key == pygame.K_f and not show_bag and not show_scroll and not show_aquarium and not show_intro:
                    show_dev_spawn = not show_dev_spawn; show_dev_add = False; show_dev_reset = False
                    ...
                    elif show_dev_spawn:           show_dev_spawn = False
                ...
                if event.button == 1:
                    ...
                    if show_online:
                        ...
                        # 화면 페이지 토글
                            ...
                            if pygame.Rect(SL_BGM[0]+SL_BGM[2]-44, SL_BGM[1]-30, 44, 22).collidepoint(mx,my):
                        ...
                        if pygame.Rect(15,12,75,28).collidepoint(mx,my):
                            show_ranking = False
                        elif RANKING_NICK_BTN.collidepoint(mx,my):
                            show_ranking = False
                            nick_changing = True
                            nickname_text = ''; ime_composition = ''
                            show_nickname_input = True
                            pygame.key.start_text_input()
                    elif aquarium_adding:
                        aq_drag_y = my
                    if not show_online and show_dev_spawn:
                        if pygame.Rect(15,12,75,28).collidepoint(mx,my):
                            show_dev_spawn = False
                        elif pygame.Rect(WIDTH//2-40,42,80,26).collidepoint(mx,my):
                            dev_spawn_idx = None; _dev_spawn_idx = None
                    ...
                    if not show_online and show_dev_add:
                        ...
                        if AQ_BACK_RECT.collidepoint(mx, my):
                            ...
                            for slot5 in range(len(JELLY_NAMES)):
                                ...
                                if abs(mx-cx5)<CW5//2 and abs(my-cy5)<CH5//2:
                                    ...
                                    save_game(inventory,_current_stage,cult_docs,aquarium,player_nickname,bgm_vol,sfx_vol)
                                    acquire_msg = f'{JELLY_NAMES[slot5]} 어항에 추가됐어!'
                                    acquire_msg_timer = 150
                        ...
                        if scroll_doc_detail is not None:
                            ...
                            # 카드 클릭 감지 (스크롤 오프셋 반영)
                            CLIP_TOP_C = 54
                            cy_card = CLIP_TOP_C - cult_doc_scroll_y
                            clicked_doc = None
                        ...
                        if context.get_kill_rect().collidepoint(mx, my):
                            ...
                            if j_killed.design_idx==1 and cult_docs.get(2,0)==0 and random.random()<0.40:
                                ...
                                if doc_timer<=0:
                                    ...
                                    doc_timer=180
                                save_game(inventory, _current_stage, cult_docs, aquarium, player_nickname)
                            # 악마 배양서: 천사(8) 또는 화난(15) 죽일 때 15%
                            if j_killed.design_idx == 8 and cult_docs.get(5,0)==0 and random.random()<0.15:
                                cult_docs[5]=1; has_new_doc=True
                                cult_doc_drops.append(CultDocDrop(j_killed.x, j_killed.y))
                                if doc_timer<=0:
                                    doc_msg='???배양서를 획득했습니다. 어떤 배양서를 얻었는지 확인해보세요.'
                                    doc_timer=180
                                save_game(inventory, _current_stage, cult_docs, aquarium, player_nickname)
                            # 아기 배양서: 개구리모자(2) 또는 고양이(5) 죽일 때 20%
                            if j_killed.design_idx in (2, 5) and cult_docs.get(4,0)==0 and random.random()<0.20:
                                cult_docs[4]=1; has_new_doc=True
                                cult_doc_drops.append(CultDocDrop(j_killed.x, j_killed.y))
                                if doc_timer<=0:
                                    doc_msg='???배양서를 획득했습니다. 어떤 배양서를 얻었는지 확인해보세요.'
                                    doc_timer=180
                                save_game(inventory, _current_stage, cult_docs, aquarium, player_nickname)
                            # 혼돈 배양서: 천사(8) 5마리 + 악마(29) 5마리 처치 시 드롭
                            if j_killed.design_idx in (8, 29):
                                _chaos_kill_count[j_killed.design_idx] = _chaos_kill_count.get(j_killed.design_idx, 0) + 1
                            if (cult_docs.get(6,0)==0
                                    and _chaos_kill_count.get(8,0)>=5
                                    and _chaos_kill_count.get(29,0)>=5):
                                cult_docs[6]=1; has_new_doc=True
                                cult_doc_drops.append(CultDocDrop(j_killed.x, j_killed.y))
                                if doc_timer<=0:
                                    doc_msg='???배양서를 획득했습니다. 어떤 배양서를 얻었는지 확인해보세요.'
                                    doc_timer=180
                                save_game(inventory, _current_stage, cult_docs, aquarium, player_nickname)
                            # 희노애락 배양서: 화난(15) 또는 슬픈(34) 죽일 때 5%
                            if j_killed.design_idx in (15, 34) and cult_docs.get(7,0)==0 and random.random()<0.05:
                                cult_docs[7]=1; has_new_doc=True
                                cult_doc_drops.append(CultDocDrop(j_killed.x, j_killed.y))
                                if doc_timer<=0:
                                    doc_msg='???배양서를 획득했습니다. 어떤 배양서를 얻었는지 확인해보세요.'
                                    doc_timer=180
        ...
        if show_online:
            ...
            if True:
                ...
                # 반 가면 착용 이동 시 흰 빤짝이 스폰
                if equipped_item == 'half_mask' and any(online_keys.values()) and not online_chat_active:
                    if random.random() < 0.30:
                        _spawn_sparkle(online_x, online_y)
            # 위치 동기화
            online_sync_t += 1
            if online_sync_t >= 2:
                online_sync_t = 0
        ...
        if show_nickname_input and not show_intro:
            ...
            draw_nickname_input(screen, nickname_text + ime_composition, cursor_blink<30, changing=nick_changing)
        if show_dev_reset:
        ...
        if DEV_MODE and dev_spawn_idx is not None and not show_dev_spawn:
            ...
            _sb = pygame.Surface((_st.get_width()+8,_st.get_height()+4),pygame.SRCALPHA)
            _sb.fill((20,10,0,180)); screen.blit(_sb,(0,HEIGHT-_st.get_height()-6))
            screen.blit(_st,(4,HEIGHT-_st.get_height()-4))
        # 어항 물고기 업데이트 (항상)
        ...
        if show_scroll:
            if scroll_doc_detail is not None:
                ...
                draw_cult_doc_list(screen, cult_docs, cult_doc_scroll_y)
        if show_bag:
            if inv_detail is not None:
        ...
        if acquire_msg_timer > 0:
            _a3 = min(255, acquire_msg_timer * 12) if acquire_msg_timer < 22 else 255
