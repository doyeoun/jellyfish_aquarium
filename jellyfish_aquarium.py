...
JELLY_NAMES = ['파란 해파리', '분홍 해파리', '개구리 모자 해파리',
               ...
               '선인장 해파리', '눈사람 해파리', '황금 해파리', '무지개 해파리', '파분 해파리', '푸딩 해파리', '소다맛 푸딩 해파리', '벚꽃 해파리', '쌍둥이 해파리', '아기 해파리', '악마 해파리', '혼돈 해파리', '새싹 해파리']
...
class AquariumFish:
    ...
    def draw(self, surf):
        ...
        # 새싹 해파리 새싹 그리기 (어항)
        if self.design_idx == 31:
            _draw_sprout(surf, x, y - bh//2 + int(bh*0.08) + int(math.sin(self.pulse*1.2)*2), bw, bh)
...
def draw_wardrobe_item_icon(surf, cx, cy, item_id, unlocked=True):
    ...
    if item_id == 'angel_halo':
        ...
        pygame.draw.rect(surf, _id, (cx-1, cy+2, 3, 8))
        for _sx, _sy in ((-8, -2), (5, -2)):
            pygame.draw.ellipse(surf, _ic, (cx+_sx, cy+_sy, 8, 11))
            pygame.draw.ellipse(surf, _id, (cx+_sx, cy+_sy, 8, 11), 1)
            pygame.draw.ellipse(surf, _ih, (cx+_sx+2, cy+_sy+2, 4, 5))
    elif item_id == 'baby':
...
class Jellyfish:
    ...
    def _apply_type(self):
        ...
        if self.design_idx == 28:
            ...
            self.tc = (201, 230, 105)  # 새싹 연두색
        self.BH         = len(defn['art'])
        self.body_color = defn['cmap'].get('M', self.tc)  # 눈 가리기용
        if self.design_idx == 31: self.body_color = (201, 230, 105)  # 새싹 본체색
