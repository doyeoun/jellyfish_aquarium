...
JELLY_NAMES = ['파란 해파리', '분홍 해파리', '개구리 모자 해파리',
               ...
               '선인장 해파리', '눈사람 해파리', '황금 해파리', '무지개 해파리', '파분 해파리', '푸딩 해파리', '소다맛 푸딩 해파리', '벚꽃 해파리', '쌍둥이 해파리', '아기 해파리', '악마 해파리']
...
class AquariumFish:
    ...
    def draw(self, surf):
        ...
        # 촉수
        tc  = (222,158,175) if self.design_idx==28 else (90,15,170) if self.design_idx==29 else JELLY_DEFS[bi]['tc']
...
def _draw_demon_wings(surf, cx, cy, bw, bh, frozen=False, bright=False):
...
def draw_cult_doc_list(surf, cult_docs):
    ...
    for doc_type in sorted(cult_docs):
        ...
        if result_slot is not None:
            ...
            if result_slot == 29: _draw_demon_wings(surf, _ms_x+17, _ms_y+13, 34, 26)
...
class Jellyfish:
    ...
    def _apply_type(self):
        ...
        if self.design_idx == 28:
            self.tc = (222, 158, 175)  # 몸통과 어울리는 핑크
        elif self.design_idx == 29:
            self.tc = (90, 15, 170)   # 밝은 보라
    ...
    def draw(self, surf):
        ...
        if self.is_dead:
            ...
            xs    = max(3, bw // 12)
            cover = max(2, bw // 18)
            x_col = (200, 20, 20) if self.design_idx == 29 else (22, 15, 15)
            for ex2 in (x - bw//7, x + bw//7):
                if self.design_idx not in (9, 29):  # 유령/악마는 원 제거
